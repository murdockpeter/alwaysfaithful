...
- [x] Model scale (0.62x) and material settings tuned empirically against captures — the native models are ~2 world units tall (a standing human figure) and read as a small standing figure with a visible weapon and headgear at typical tactical zoom after two iteration passes (first attempt was too small and too dark).
- Not done: PLANMC Mortar Team's distinct silhouette wasn't screenshot-verified (same gap as Group C's illustrated/symbol barrel-prop distinction); USMC Officer/Corpsman/M249 Gunner/MAAWS Gunner/EW Operator and the PLANMC/USMC vehicles and drones already sitting in the source pack are not yet imported — only the three roles Always Faithful currently fields.
