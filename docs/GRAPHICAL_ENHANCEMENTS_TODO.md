...
- [x] Follow-up: rifle-role figures now render as a small cluster of four (0.38x each, scattered/yaw-jittered like `TacticalFormationView`'s own three-element wedge) instead of one dominant soldier — reads as a squad, not a clone-stamped giant, and reuses the single rifleman sculpt the way a real tabletop stand mounts several identical minis. The support-role Mortar Team keeps its single instance since that sculpt is already a multi-figure crew. Confirmed by capture. PLANMC Mortar Team's distinct silhouette is now also confirmed (an earlier unrelated capture happened to catch an Identified support contact).
- [x] Follow-up: the cluster now spreads noticeably wider (roughly doubled offset magnitudes, 0.34x member scale) for a "small tabletop vignette" look rather than a tight clump, and each scene picked up a vehicle/drone prop as set dressing — a USMC Black Hornet drone hovering over the USMC scene (the pack has no USMC ground vehicle, only this drone and faction-unclear generic quadcopters), and a PLANMC ZBL-09 wheeled IFV beside the PLA support-team crew only, not the rifle squad. The ZBL-09's texture looked like a Unity "missing texture" checkerboard at first glance; it's actually a legitimate blue digital naval camouflage pattern, plausible for a PLA *Navy* Marine Corps vehicle, confirmed by inspecting the source PNG directly. Confirmed in-scene by capture; the drone's presence was confirmed via the player log (no missing-model warning) rather than an unambiguous screenshot, since it's a deliberately small, subtle accent that's hard to isolate at typical zoom.
- Not done: the cluster figures' facing direction is an untested guess, not checked against movement direction; the vehicle/drone scale and position were tuned once, not iterated further; USMC Officer/Corpsman/M249 Gunner/MAAWS Gunner/EW Operator and the PLANMC EQ2050 light vehicle already sitting in the source pack are not yet imported — only the three roles and two vignette props Always Faithful currently uses.
