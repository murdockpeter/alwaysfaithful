...
- **NonCommercial**: Always Faithful must not be sold or otherwise monetized while these models remain in the project.
- **ShareAlike**: any adaptation of these specific assets must be distributed under the same CC BY-NC-SA 4.0 terms.
- Changes made here: converted for direct use in Always Faithful's own tactical-counter presentation layer (`MiniatureCounterView`), including runtime material/texture assignment and status-tint handling specific to this game; the mesh, UVs, and painted textures themselves are unmodified from the source files.

These are placeholder-quality integration assets for Always Faithful's Miniature counter skin, not an endorsement of Always Faithful by the creator. If Always Faithful ever moves toward a commercial release, these specific files need to be replaced or separately licensed first.
