...
{
    /// <summary>
    /// Reads the version-one elevation binary currently shared with Sea of Uncertainty.
    /// The data is presentation-only until a tactical-resolution source is imported.
    /// </summary>
    public sealed class GeographicElevationGrid
    {
        ...
        public float SampleMetres(double longitude, double latitude)
        {
            ...
            float south = Mathf.Lerp(Elevation(column, row), Elevation(column + 1, row), columnBlend);
            float north = Mathf.Lerp(Elevation(column, row + 1), Elevation(column + 1, row + 1), columnBlend);
        ...
        public static bool TryLoad(TextAsset asset, out GeographicElevationGrid grid, out string error)
        {
            ...
            if (asset == null)
            {
                error = "Elevation resource is missing.";
                return false;
            }
            ...
            try
            {
                ...
                using (var reader = new BinaryReader(stream, Encoding.ASCII, false))
                {
                    ...
                    {
                        error = "Elevation resource has an invalid signature.";
                        return false;
                    }
                    ...
                    if (stream.Length - stream.Position != count * sizeof(short))
                    {
                        error = "Elevation resource sample count does not match its dimensions.";
                        return false;
                    }
        ...
        private short Elevation(int column, int row)
