...
{
    internal enum TacticalSpecialOrder { None, Face, AreaSuppress, Smoke, Assault, CallForFire, Search, Withdraw, DetectObstacle, HastyBreach, DeliberateBreach, MortarFire }
    ...
    public sealed class AlwaysFaithfulPrototype : MonoBehaviour
    {
        ...
        private readonly List<TacticalUnitState> tacticalFriendlyStates = new List<TacticalUnitState>();
        private readonly Dictionary<string, TacticalFriendlyView> tacticalFriendlyViews = new Dictionary<string, TacticalFriendlyView>();
        private readonly Dictionary<string, TacticalWeaponState> tacticalFriendlyWeapons = new Dictionary<string, TacticalWeaponState>();
        private readonly Dictionary<UnitCounterView, TacticalFriendlyView> tacticalFriendlyByCounter = new Dictionary<UnitCounterView, TacticalFriendlyView>();
        ...
        private string tacticalCareerPath;
        private TacticalCompanyCareerState tacticalCareer;
        private TacticalQuickBattleConfiguration requestedQuickBattle;
        ...
        private TacticalCompanyPreset tacticalCompanyPreset = TacticalCompanyPreset.Balanced;
        private bool tacticalForcePanelOpen;
        private Vector2 tacticalCompanyScroll;
        private bool tacticalDeploymentActive;
        private TacticalUnitState tacticalDeploymentUnit;
        private bool tacticalObstaclePlacementActive;
        private int tacticalDefensiveObstacleBudget;
        private TacticalObstacleType tacticalDefensiveObstacleType = TacticalObstacleType.Minefield;
        ...
        private readonly Dictionary<string, GameObject> tacticalObstacleViews = new Dictionary<string, GameObject>();
        private readonly Dictionary<string, GameObject> tacticalFireAimViews = new Dictionary<string, GameObject>();
        private readonly Dictionary<HexCoord, GameObject> tacticalIlluminationViews = new Dictionary<HexCoord, GameObject>();
        private TacticalFireMissionType tacticalFireMissionType = TacticalFireMissionType.HighExplosive;
        ...
        private bool automatedFireManeuverRegression;
        private bool automatedCombatPowerRegression;
        private bool automatedLongevityRegression;
        private bool automatedConcealmentMoraleRegression;
        private bool automatedObstaclesRegression;
        private bool automatedPlannedFiresRegression;
        ...
        private bool automatedCompanyRegression;
        ...
        private bool hudCollapsed;
        private string hoveredOrderTooltip;
        ...
        private sealed class TacticalFriendlyView
        {
            public TacticalUnitState State;
            public TacticalWeaponState Weapon;
            public TacticalPlatoonRole Role;
            public UnitCounterView Counter;
            public TacticalFormationView Formation;
            public NatoSymbolView Symbol;
            public MiniatureCounterView Miniature;
            public GameObject IllustratedDetail;
            public GameObject SymbolDetail;
            public GameObject MiniatureDetail;
        }
        ...
        private void Awake()
        {
            ...
            automatedFireManeuverRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--fire-maneuver-regression") >= 0;
            automatedCombatPowerRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--combat-power-regression") >= 0;
            automatedLongevityRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--longevity-regression") >= 0;
            automatedConcealmentMoraleRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--concealment-morale-regression") >= 0;
            automatedObstaclesRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--obstacles-regression") >= 0;
            automatedPlannedFiresRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--planned-fires-regression") >= 0;
            ...
            automatedCompanyRegression = Array.IndexOf(Environment.GetCommandLineArgs(), "--company-regression") >= 0;
            ...
            tacticalCareerPath = IsAutomatedRun()
                ? Path.Combine(Path.GetTempPath(), "always-faithful-automated-career.json")
                : Path.Combine(Path.GetDirectoryName(battalionStatusPath) ?? Application.persistentDataPath, "always-faithful-company-career.json");
            if (IsAutomatedRun() || !TryLoadTacticalCareer(tacticalCareerPath, out tacticalCareer)) tacticalCareer = new TacticalCompanyCareerState();
            ...
            string quickBattleArgument = Array.Find(Environment.GetCommandLineArgs(), value => value.StartsWith("--quick-battle=", StringComparison.Ordinal));
            if (quickBattleArgument != null)
            {
                string quickBattlePath = quickBattleArgument.Substring("--quick-battle=".Length);
                try { requestedQuickBattle = JsonUtility.FromJson<TacticalQuickBattleConfiguration>(File.ReadAllText(quickBattlePath)); }
                catch (Exception exception) { Debug.LogWarning($"ALWAYS_FAITHFUL_QUICK_BATTLE_LOAD_FAILED path={quickBattlePath} error={exception.Message}"); }
            }
            LoadGeography();
            ...
            if (automatedFireManeuverRegression) StartCoroutine(RunFireManeuverRegression());
            if (automatedCombatPowerRegression) StartCoroutine(RunCombatPowerRegression());
            if (automatedLongevityRegression) StartCoroutine(RunLongevityRegression());
            if (automatedConcealmentMoraleRegression) StartCoroutine(RunConcealmentMoraleRegression());
            if (automatedObstaclesRegression) StartCoroutine(RunObstaclesRegression());
            if (automatedPlannedFiresRegression) StartCoroutine(RunPlannedFiresRegression());
            ...
            if (automatedCompanyRegression) StartCoroutine(RunCompanyRegression());
        ...
        private bool IsAutomatedRun()
            => automatedCapture || automatedMovementRegression || automatedTacticalRegression || automatedTacticalMovementRegression ||
               ...
               automatedReactionRegression || automatedFireManeuverRegression || automatedCombatPowerRegression || automatedLongevityRegression || automatedConcealmentMoraleRegression || automatedObstaclesRegression || automatedPlannedFiresRegression || automatedEnemyTurnRegression || automatedCoverRegression || automatedVictoryRegression ||
               ...
               automatedSaveRestoreRegression || automatedSaveRestoreCapture || automatedFeedbackRegression || automatedScenarioRegression || automatedCompanyRegression ||
        ...
        {
            if (mapTransitionActive || campaignBriefingActive || operationalBriefingActive || supportCardModalActive ||
                tacticalForcePanelOpen || tacticalDeploymentActive || resultScreenActive || settingsPanelOpen) return;
        ...
        private void TryLoadBattleRequest(string path)
        {
            ...
            tacticalOperationalEngagement = false;
            tacticalOperationalEnemyBattalion = null;
            if (!File.Exists(path))
            {
        ...
        private void DismissSupportCardModal(TacticalSupportCard playedCard)
        {
            ...
            tacticalDeploymentActive = false;
            tacticalDeploymentUnit = null;
            campaignBriefingActive = true;
            StartCoroutine(ShowCampaignBriefingThenDismiss());
        }
        ...
        private void SaveTacticalBattle(string outputPath = null)
        {
            ...
            var state = new TacticalBattleSaveState
            {
                ...
                SelectedFriendlyUnitId = tacticalUnitState.Id,
                CompanyPreset = tacticalCompanyPreset,
                ...
                IsOperationalEngagement = tacticalOperationalEngagement,
                OperationalParentBattalionId = tacticalOperationalEngagement ? activeOperationalBattalion?.Id : null,
                OperationalEnemyBattalionId = tacticalOperationalEngagement ? tacticalOperationalEnemyBattalion?.Id : null
            };
            state.FriendlyUnits.AddRange(tacticalFriendlyStates);
            foreach (KeyValuePair<string, TacticalWeaponState> pair in tacticalFriendlyWeapons)
                state.FriendlyWeapons.Add(new TacticalFriendlyWeaponEntry { UnitId = pair.Key, Weapon = pair.Value });
        ...
        private bool TryLoadTacticalBattle(string path, out string error)
        {
            ...
            tacticalOperationalEngagement = state.IsOperationalEngagement;
            if (tacticalOperationalEngagement)
            {
                OperationalBattalionState parent = operationalFriendlyBattalions.Find(item => item.Id == state.OperationalParentBattalionId);
                if (parent == null)
                {
                    error = "Saved operational parent battalion is no longer present";
                    return false;
                }
                SelectOperationalBattalion(operationalFriendlyViews[parent.Id]);
                tacticalOperationalEnemyBattalion = operationalEnemyBattalions.Find(item => item.Id == state.OperationalEnemyBattalionId);
            }
            else tacticalOperationalEnemyBattalion = null;
        ...
        private bool TryLoadTacticalCareer(string path, out TacticalCompanyCareerState career)
        {
            career = null;
            if (!File.Exists(path)) return false;
            try
            {
                career = JsonUtility.FromJson<TacticalCompanyCareerState>(File.ReadAllText(path));
                return career != null && career.SchemaVersion == 1 && career.Platoons != null;
            }
            catch (Exception exception)
            {
                Debug.LogWarning($"ALWAYS_FAITHFUL_CAREER_LOAD_FAILED path={path} error={exception.Message}");
                return false;
            }
        }

        private void SaveTacticalCareer()
        {
            if (tacticalCareer == null || string.IsNullOrEmpty(tacticalCareerPath)) return;
            string tempPath = tacticalCareerPath + ".tmp";
            try
            {
                string directory = Path.GetDirectoryName(tacticalCareerPath);
                if (!string.IsNullOrEmpty(directory)) Directory.CreateDirectory(directory);
                File.WriteAllText(tempPath, JsonUtility.ToJson(tacticalCareer, true));
                if (File.Exists(tacticalCareerPath)) File.Delete(tacticalCareerPath);
                File.Move(tempPath, tacticalCareerPath);
            }
            catch (Exception exception)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_CAREER_SAVE_FAILED path={tacticalCareerPath} error={exception.Message}");
            }
        }
        ...
        private OperationalScenarioState CreateOperationalScenario()
        {
            ...
            scenario.FriendlyBattalions.Add(CreateOperationalBattalion("usmc-bn-1-4", "1st Battalion, 4th Marines", "1/4", OperationalSide.Usmc,
                FindDistinctDeploymentHex(new HexCoord(21, 25), used), objective, OperationalBattalionConfiguration.UsmcReinforcedAssault,
            ...
            scenario.FriendlyBattalions.Add(CreateOperationalBattalion("usmc-bn-2-4", "2d Battalion, 4th Marines", "2/4", OperationalSide.Usmc,
                FindDistinctDeploymentHex(new HexCoord(18, 34), used), objective, OperationalBattalionConfiguration.UsmcLittoralScreen,
            ...
            if (operationalPersistenceEnabled && battalionStatus != null)
            {
                ...
                foreach (OperationalPlatoonState platoon in scenario.FriendlyBattalions[0].Platoons)
                    platoon.Strength = Math.Min(platoon.MaximumStrength, battalionStatus.Strength);
            }
            scenario.EnemyBattalions.Add(CreateOperationalBattalion("pla-amphib-bn-1", "PLA 1st Amphibious Combined-Arms Battalion", "1 ACB", OperationalSide.Pla,
                FindDistinctDeploymentHex(new HexCoord(38, 52), used), objective, OperationalBattalionConfiguration.PlaAmphibiousCombinedArms,
            ...
            scenario.EnemyBattalions.Add(CreateOperationalBattalion("pla-amphib-bn-2", "PLA 2d Amphibious Combined-Arms Battalion", "2 ACB", OperationalSide.Pla,
                FindDistinctDeploymentHex(new HexCoord(43, 42), used), objective, OperationalBattalionConfiguration.PlaAmphibiousCombinedArms,
                "Three maneuver companies", "Firepower company", "Service support company"));
            scenario.EnemyBattalions.Add(CreateOperationalBattalion("pla-recon-bn", "PLA Reconnaissance Battalion", "RECON", OperationalSide.Pla,
                FindDistinctDeploymentHex(new HexCoord(35, 65), used), objective, OperationalBattalionConfiguration.PlaReconnaissance,
        ...
        private static OperationalBattalionState CreateOperationalBattalion(string id, string displayName, string shortName,
            OperationalSide side, HexCoord position, HexCoord objective, OperationalBattalionConfiguration configuration,
            params string[] subordinateUnits)
        {
            var battalion = new OperationalBattalionState
            {
                Id = id,
            ...
            battalion.Platoons = OperationalScenarioRules.BuildPlatoons(id, shortName, configuration);
            return battalion;
        }
        ...
        private bool TryLoadOperationalScenario(string path, out OperationalScenarioState scenario)
        {
            ...
            try
            {
                ...
                if (parsed == null || parsed.FriendlyBattalions == null || parsed.EnemyBattalions == null) return false;
                foreach (OperationalBattalionState battalion in parsed.FriendlyBattalions)
                {
                    if (battalion.Platoons == null || battalion.Platoons.Count == 0)
                        battalion.Configuration = battalion.Id == "usmc-bn-2-4"
                            ? OperationalBattalionConfiguration.UsmcLittoralScreen
                            : OperationalBattalionConfiguration.UsmcReinforcedAssault;
                    OperationalScenarioRules.EnsurePlatoonOrderOfBattle(battalion);
                ...
                {
                    if (battalion.Platoons == null || battalion.Platoons.Count == 0)
                        battalion.Configuration = battalion.Id == "pla-recon-bn"
                            ? OperationalBattalionConfiguration.PlaReconnaissance
                            : OperationalBattalionConfiguration.PlaAmphibiousCombinedArms;
                    OperationalScenarioRules.EnsurePlatoonOrderOfBattle(battalion);
                    if (!cells.ContainsKey(battalion.Position)) return false;
                }
        ...
        private void SaveOperationalScenario()
        {
            ...
            // During first-time creation this save occurs before BuildUnit has
            // copied the scenario intelligence into the runtime dictionaries.
            // Preserve the freshly-created contact memory in that case.
            if (operationalContacts.Count > 0)
                operationalScenario.EnemyContacts = new List<OperationalContactState>(operationalContacts.Values);
            if (operationalReconMarkers.Count > 0 || operationalScenario.ReconMarkers == null)
        ...
        private void EnterTacticalMap(HexCellView parentCell, bool animate, TacticalBattleSaveState restore = null, bool hasKnownEnemyContact = true)
        {
            ...
            bool freshOperationalScenario = restore == null && activeBattleRequest == null && tacticalOperationalEngagement && !resumingInProgress;
            bool freshStandaloneScenario = restore == null && activeBattleRequest == null && !tacticalOperationalEngagement && !resumingInProgress;
            if (freshOperationalScenario)
            {
                string engagementKey = operationalScenario.ScenarioId + ":" + activeOperationalBattalion.Id + ":" +
                    (tacticalOperationalEnemyBattalion?.Id ?? "clear") + ":" + parentCell.Coord;
                standaloneScenarioSeed = TacticalDirectFire.CreateSeed(engagementKey, operationalScenario.TurnNumber, 1);
            }
            else if (freshStandaloneScenario)
                standaloneScenarioSeed = scenarioSeedOverride ?? (requestedQuickBattle != null && requestedQuickBattle.Seed > 0
                    ? requestedQuickBattle.Seed : UnityEngine.Random.Range(1, int.MaxValue));
        ...
        private void BuildTacticalBattlefield(HexCellView parentCell, TacticalBattleSaveState restore = null, bool hasKnownEnemyContact = true)
        {
            ...
            localCells.Clear();
            tacticalSmokeViews.Clear();
            tacticalObstacleViews.Clear();
            ...
            if (tacticalBattlefield.QuickBattle == null) tacticalBattlefield.QuickBattle = new TacticalQuickBattleConfiguration();
            if (restore == null && activeBattleRequest == null && !tacticalOperationalEngagement && requestedQuickBattle != null) tacticalBattlefield.QuickBattle = requestedQuickBattle;
            if (restore == null && activeBattleRequest == null && !tacticalOperationalEngagement)
                tacticalCompanyPreset = tacticalBattlefield.QuickBattle.CompanyPreset;
            if (tacticalBattlefield.SecondaryObjectives == null) tacticalBattlefield.SecondaryObjectives = new List<TacticalSecondaryObjectiveState>();
            if (tacticalBattlefield.Tutorial == null) tacticalBattlefield.Tutorial = new TacticalTutorialState();
            ...
            BuildTacticalUnits(restore);
            BuildTacticalMovementVisuals();
            BuildTacticalLosVisuals();
            BuildTacticalFireVisuals();
            if (tacticalBattlefield.ActiveSmokeMarkers == null) tacticalBattlefield.ActiveSmokeMarkers = new List<TacticalSmokeMarker>();
            ...
            if (tacticalBattlefield.FireSupport == null) tacticalBattlefield.FireSupport = new TacticalFireSupportState();
            if (tacticalBattlefield.PlannedFireMissions == null) tacticalBattlefield.PlannedFireMissions = new List<TacticalPlannedFireMission>();
            if (tacticalBattlefield.PlannedFireEvents == null) tacticalBattlefield.PlannedFireEvents = new List<TacticalPlannedFireEvent>();
            if (tacticalBattlefield.ActiveIlluminationMarkers == null) tacticalBattlefield.ActiveIlluminationMarkers = new List<TacticalIlluminationMarker>();
            foreach (TacticalSmokeMarker marker in tacticalBattlefield.ActiveSmokeMarkers)
            {
                ...
                smokeCell.SmokeExpiresAfterTurn = marker.ExpiresAfterTurn;
                CreateOrRefreshSmokeView(marker.Position);
            }
            BuildTacticalContacts(restore, hasKnownEnemyContact);
            ApplyCounterSkin();
            BuildTacticalObjective(restore);
            InitializeTacticalObstacles(restore != null);
            RefreshPlannedFireViews();
            RefreshTacticalObservation();
        ...
        private void InitializeTacticalObstacles(bool restoring)
        {
            if (tacticalBattlefield.Obstacles == null) tacticalBattlefield.Obstacles = new List<TacticalObstacleState>();
            if (tacticalBattlefield.ObstacleEvents == null) tacticalBattlefield.ObstacleEvents = new List<TacticalObstacleEvent>();
            if (!restoring && tacticalBattlefield.Obstacles.Count == 0 && tacticalEnemyStates.Count > 0 &&
                tacticalObjective.MissionType != TacticalMissionType.Defend && !IsAutomatedRun())
            {
                var excluded = new HashSet<HexCoord>();
                foreach (TacticalUnitState friendly in tacticalFriendlyStates) excluded.Add(friendly.Position);
                foreach (TacticalUnitState enemy in tacticalEnemyStates) excluded.Add(enemy.Position);
                tacticalBattlefield.Obstacles.AddRange(
                    TacticalObstacles.GenerateDefensiveLayout(tacticalBattlefield, tacticalObjective.ObjectiveHex, excluded));
            }
            foreach (TacticalObstacleState obstacle in tacticalBattlefield.Obstacles)
            {
                if (localMovementBoard.TryGetValue(obstacle.Position, out TacticalMovementCell cell))
                    TacticalObstacles.ApplyToCell(cell, obstacle);
                RefreshTacticalObstacleView(obstacle);
            }
        }

        private TacticalObstacleState FindTacticalObstacle(HexCoord position)
            => tacticalBattlefield?.Obstacles?.Find(item => item.Position.Equals(position));

        private void RefreshTacticalObstacleView(TacticalObstacleState obstacle)
        {
            if (obstacle == null) return;
            if (tacticalObstacleViews.TryGetValue(obstacle.Id, out GameObject oldView))
            {
                Destroy(oldView);
                tacticalObstacleViews.Remove(obstacle.Id);
            }
            if (obstacle.UsmcIntelligence == TacticalObstacleIntelligence.Hidden ||
                !localCells.TryGetValue(obstacle.Position, out HexCellView cell)) return;

            GameObject marker = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            marker.name = "Obstacle " + obstacle.Id;
            marker.layer = LayerMask.NameToLayer("Ignore Raycast");
            marker.transform.SetParent(tacticalRoot.transform, false);
            marker.transform.position = cell.transform.position + Vector3.up * (CellSurfaceOffset + .06f);
            marker.transform.localScale = new Vector3(.58f, .025f, .58f);
            Color color = obstacle.IsBreached ? new Color(.28f, .82f, .62f, .72f) :
                string.Equals(obstacle.OwnerSide, "USMC", StringComparison.OrdinalIgnoreCase) ? new Color(.24f, .62f, .94f, .76f) :
                obstacle.UsmcIntelligence == TacticalObstacleIntelligence.Suspected ? new Color(1f, .73f, .18f, .62f) :
                new Color(.92f, .22f, .14f, .78f);
            marker.GetComponent<MeshRenderer>().sharedMaterial = NewOverlayMaterial(color);
            Destroy(marker.GetComponent<Collider>());

            GameObject labelObject = new GameObject("Obstacle Label");
            labelObject.layer = LayerMask.NameToLayer("Ignore Raycast");
            labelObject.transform.SetParent(marker.transform, false);
            labelObject.transform.localPosition = new Vector3(0f, .7f, 0f);
            labelObject.transform.localRotation = Quaternion.Euler(90f, 0f, 0f);
            TextMesh label = labelObject.AddComponent<TextMesh>();
            label.text = obstacle.IsBreached ? "LANE" :
                obstacle.UsmcIntelligence == TacticalObstacleIntelligence.Suspected ? "?" :
                obstacle.UsmcIntelligence == TacticalObstacleIntelligence.Detected ? "OBS" :
                obstacle.Type == TacticalObstacleType.Minefield ? "MINE" :
                obstacle.Type == TacticalObstacleType.Wire ? "WIRE" : "BLOCK";
            label.anchor = TextAnchor.MiddleCenter;
            label.alignment = TextAlignment.Center;
            label.fontSize = 42;
            label.characterSize = .055f;
            label.color = Color.white;
            tacticalObstacleViews[obstacle.Id] = marker;
        }

        private bool ResolveTacticalObstacleEntry(TacticalUnitState unit, HexCoord position, string movingSide, out string summary)
        {
            summary = null;
            TacticalObstacleState obstacle = FindTacticalObstacle(position);
            if (obstacle == null) return false;
            TacticalObstacleEncounter encounter = TacticalObstacles.Enter(
                obstacle, unit.MovementPosture, movingSide,
                TacticalDirectFire.CreateSeed(tacticalBattlefield.BattlefieldId, turnState.TurnNumber, tacticalEventSequence + 1));
            if (!encounter.Halted) return false;

            if (localMovementBoard.TryGetValue(position, out TacticalMovementCell cell))
                TacticalObstacles.ApplyToCell(cell, obstacle);
            if (encounter.Suppression > 0) unit.ApplySuppressionPoints(encounter.Suppression);
            if (encounter.StrengthDamage > 0) RecordStrengthDamage(unit, encounter.StrengthDamage, "Minefield");
            RefreshTacticalObstacleView(obstacle);
            summary = obstacle.Type == TacticalObstacleType.Minefield
                ? $"Minefield contact: {encounter.StrengthDamage} strength and {encounter.Suppression} suppression"
                : obstacle.Type == TacticalObstacleType.Wire
                    ? $"Wire belt halted movement; {encounter.Suppression} suppression"
                    : "Roadblock halted movement";
            if (encounter.NewlyIdentified)
                RecordTacticalObstacleEvent(TacticalObstacleEventKind.Identified, unit, obstacle,
                    encounter.Suppression, encounter.StrengthDamage, obstacle.Type + " identified by contact");
            RecordTacticalObstacleEvent(TacticalObstacleEventKind.MovementHalted, unit, obstacle,
                encounter.Suppression, encounter.StrengthDamage, summary);
            return true;
        }

        private TacticalObstacleEvent RecordTacticalObstacleEvent(TacticalObstacleEventKind kind, TacticalUnitState unit,
            TacticalObstacleState obstacle, int suppression, int damage, string summary, int actionPointCost = 0,
            int engineerSupplyBefore = 0, int engineerSupplyAfter = 0, bool succeeded = true)
        {
            var entry = new TacticalObstacleEvent
            {
                Sequence = ++tacticalEventSequence,
                BattlefieldId = tacticalBattlefield.BattlefieldId,
                Turn = turnState.TurnNumber,
                ...
                Position = obstacle?.Position ?? unit?.Position ?? default,
                Type = obstacle?.Type ?? TacticalObstacleType.None,
                Intelligence = obstacle?.UsmcIntelligence ?? TacticalObstacleIntelligence.Hidden,
                SuppressionApplied = suppression,
                StrengthDamage = damage,
                ActionPointCost = actionPointCost,
                EngineerSupplyBefore = engineerSupplyBefore,
                EngineerSupplyAfter = engineerSupplyAfter,
                Succeeded = succeeded,
                Summary = summary
            };
            tacticalBattlefield.ObstacleEvents.Add(entry);
            Debug.Log($"ALWAYS_FAITHFUL_OBSTACLE_EVENT sequence={entry.Sequence} kind={kind} hex={entry.Position} type={entry.Type} summary={summary}");
            return entry;
        }

        private void RefreshPlannedFireViews()
        {
            foreach (GameObject view in tacticalFireAimViews.Values) if (view != null) Destroy(view);
            tacticalFireAimViews.Clear();
            foreach (GameObject view in tacticalIlluminationViews.Values) if (view != null) Destroy(view);
            tacticalIlluminationViews.Clear();
            if (tacticalBattlefield?.PlannedFireMissions != null)
                foreach (TacticalPlannedFireMission mission in tacticalBattlefield.PlannedFireMissions)
                    if (mission.Status == TacticalFireMissionStatus.Pending)
                        BuildPlannedFireMarker(mission.Id, mission.RequestedHex,
                            $"{FireMissionShortLabel(mission.Type)} T-{Math.Max(0, mission.DeliveryTurn - turnState.TurnNumber)}", new Color(1f, .66f, .16f, .88f));
            if (tacticalBattlefield?.FireSupport?.HasAdjustedAimPoint == true && !tacticalFireAimViews.ContainsKey("ADJUSTED"))
                BuildPlannedFireMarker("ADJUSTED", tacticalBattlefield.FireSupport.AdjustedAimPoint,
                    $"ADJ {tacticalBattlefield.FireSupport.AdjustmentLevel}", new Color(.35f, .82f, 1f, .72f));
            if (tacticalBattlefield?.ActiveIlluminationMarkers != null)
                foreach (TacticalIlluminationMarker marker in tacticalBattlefield.ActiveIlluminationMarkers)
                    if (marker.IsActive(turnState.TurnNumber)) BuildIlluminationView(marker);
        }

        private void BuildPlannedFireMarker(string id, HexCoord position, string text, Color color)
        {
            if (!localCells.TryGetValue(position, out HexCellView cell)) return;
            GameObject marker = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            marker.name = "Planned Fire " + id;
            marker.layer = LayerMask.NameToLayer("Ignore Raycast");
            marker.transform.SetParent(tacticalRoot.transform, false);
            marker.transform.position = cell.transform.position + Vector3.up * (CellSurfaceOffset + .11f);
            marker.transform.localScale = new Vector3(.72f, .025f, .72f);
            marker.GetComponent<MeshRenderer>().sharedMaterial = NewOverlayMaterial(color);
            Destroy(marker.GetComponent<Collider>());
            GameObject labelObject = new GameObject("Fire Countdown");
            labelObject.layer = LayerMask.NameToLayer("Ignore Raycast");
            labelObject.transform.SetParent(marker.transform, false);
            ...
            label.color = Color.white;
            tacticalFireAimViews[id] = marker;
        }

        private void BuildIlluminationView(TacticalIlluminationMarker marker)
        {
            if (!localCells.TryGetValue(marker.Position, out HexCellView cell)) return;
            GameObject light = GameObject.CreatePrimitive(PrimitiveType.Sphere);
            light.name = "Illumination " + marker.Position;
            light.layer = LayerMask.NameToLayer("Ignore Raycast");
            light.transform.SetParent(tacticalRoot.transform, false);
            light.transform.position = cell.transform.position + Vector3.up * 1.65f;
            light.transform.localScale = Vector3.one * .34f;
            light.GetComponent<MeshRenderer>().sharedMaterial = NewOverlayMaterial(new Color(1f, .92f, .55f, .82f));
            Destroy(light.GetComponent<Collider>());
            tacticalIlluminationViews[marker.Position] = light;
        }

        private void ResolveDueFireMissions()
        {
            if (tacticalBattlefield?.PlannedFireMissions == null) return;
            foreach (TacticalPlannedFireMission mission in tacticalBattlefield.PlannedFireMissions)
            {
                if (mission.Status != TacticalFireMissionStatus.Pending || mission.DeliveryTurn > turnState.TurnNumber) continue;
                int seed = TacticalDirectFire.CreateSeed(tacticalBattlefield.BattlefieldId, mission.DeliveryTurn, tacticalEventSequence + 1);
                mission.ImpactHex = TacticalPlannedFires.ResolveScatter(localMovementBoard, mission.RequestedHex, mission.FinalScatterRadius, seed);
                int affected = 0;
                if (mission.Type == TacticalFireMissionType.HighExplosive || mission.Type == TacticalFireMissionType.MortarHighExplosive)
                {
                    var allUnits = new List<TacticalUnitState>(tacticalFriendlyStates);
                    allUnits.AddRange(tacticalEnemyStates);
                    foreach (TacticalUnitState unit in allUnits)
                    {
                        if (unit.Strength <= 0 || HexCoord.Distance(unit.Position, mission.ImpactHex) > TacticalPlannedFires.EffectRadiusHexes) continue;
                        int beforePoints = unit.SuppressionPoints;
                        TacticalCombatStatus beforeStatus = unit.CombatStatus;
                        int suppression = mission.Type == TacticalFireMissionType.MortarHighExplosive ? 30 : TacticalPlannedFires.HeSuppression;
                        int damage = mission.Type == TacticalFireMissionType.MortarHighExplosive ? 10 : TacticalPlannedFires.HeStrengthDamage;
                        unit.ApplySuppressionPoints(suppression);
                        tacticalBattlefield.SuppressionEvents.Add(new TacticalSuppressionEvent
                        {
                            Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId,
                            Turn = turnState.TurnNumber, UnitId = unit.Id, Cause = mission.Type.ToString(),
                            PointsBefore = beforePoints, PointsAfter = unit.SuppressionPoints,
                            StatusBefore = beforeStatus, StatusAfter = unit.CombatStatus
                        });
                        RecordStrengthDamage(unit, damage, mission.Type.ToString());
                        affected++;
                    }
                    foreach (TacticalUnitState enemy in tacticalEnemyStates) ResolvePendingFallback(enemy, tacticalFriendlyStates);
                    foreach (TacticalUnitState friendly in tacticalFriendlyStates) ResolvePendingFallback(friendly, tacticalEnemyStates);
                }
                else if (mission.Type == TacticalFireMissionType.Smoke || mission.Type == TacticalFireMissionType.MortarSmoke)
                {
                    int radius = mission.Type == TacticalFireMissionType.Smoke ? 1 : 0;
                    foreach (TacticalMovementCell cell in localMovementBoard.Values)
                    {
                        if (!cell.IsPassable || HexCoord.Distance(cell.Coord, mission.ImpactHex) > radius) continue;
                        int expires = turnState.TurnNumber + TacticalPlannedFires.SmokeDurationTurns - 1;
                        cell.SmokeExpiresAfterTurn = expires;
                        tacticalBattlefield.ActiveSmokeMarkers.RemoveAll(item => item.Position.Equals(cell.Coord));
                        tacticalBattlefield.ActiveSmokeMarkers.Add(new TacticalSmokeMarker
                        {
                            Position = cell.Coord, PlacedTurn = turnState.TurnNumber,
                            ExpiresAfterTurn = expires, SourceUnitId = mission.ObserverId
                        });
                        CreateOrRefreshSmokeView(cell.Coord);
                    }
                }
                else
                {
                    tacticalBattlefield.ActiveIlluminationMarkers.RemoveAll(item => item.Position.Equals(mission.ImpactHex));
                    tacticalBattlefield.ActiveIlluminationMarkers.Add(new TacticalIlluminationMarker
                    {
                        Position = mission.ImpactHex, PlacedTurn = turnState.TurnNumber,
                        ExpiresAfterTurn = turnState.TurnNumber + TacticalPlannedFires.IlluminationDurationTurns - 1
                    });
                }
                mission.AffectedUnits = affected;
                mission.Status = TacticalFireMissionStatus.Delivered;
                RecordPlannedFireEvent(mission, $"{mission.Type} impacted {mission.ImpactHex}; {affected} formations affected");
            }
            tacticalBattlefield.ActiveIlluminationMarkers.RemoveAll(item => !item.IsActive(turnState.TurnNumber));
            RefreshPlannedFireViews();
        }

        private void RecordPlannedFireEvent(TacticalPlannedFireMission mission, string summary)
        {
            tacticalBattlefield.PlannedFireEvents.Add(new TacticalPlannedFireEvent
            {
                Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId,
                Turn = turnState.TurnNumber, MissionId = mission.Id, ObserverId = mission.ObserverId,
                Type = mission.Type, Status = mission.Status, RequestedHex = mission.RequestedHex,
                ImpactHex = mission.ImpactHex, ScatterRadius = mission.FinalScatterRadius,
                DangerClose = mission.DangerClose, AffectedUnits = mission.AffectedUnits, Summary = summary
            });
        }

        private static string FireMissionShortLabel(TacticalFireMissionType type)
            => type == TacticalFireMissionType.HighExplosive ? "HE" :
               type == TacticalFireMissionType.Smoke ? "SMK" :
               type == TacticalFireMissionType.Illumination ? "ILL" :
               type == TacticalFireMissionType.MortarSmoke ? "M-SMK" : "M-HE";
        ...
        private void BuildTacticalObjective(TacticalBattleSaveState restore = null)
        {
            ...
            else if (tacticalOperationalEngagement)
            {
        ...
        private void UpdateObjectiveMarkerColor()
        {
            ...
            bool usmcControls = ObjectiveControlledByUsmc();
            ...
            if (!tacticalDeploymentActive && !tacticalForcePanelOpen && tacticalObjectiveControlledByUsmc.HasValue && tacticalObjectiveControlledByUsmc.Value != usmcControls)
            {
        ...
        private void BuildTacticalUnits(TacticalBattleSaveState restore = null)
        {
            tacticalFriendlyStates.Clear();
            tacticalFriendlyViews.Clear();
            tacticalFriendlyWeapons.Clear();
            tacticalFriendlyByCounter.Clear();
            tacticalCompanyPreset = restore != null ? restore.CompanyPreset : tacticalCompanyPreset;

            var states = new List<TacticalUnitState>();
            var weapons = new Dictionary<string, TacticalWeaponState>();
            var roles = new List<TacticalPlatoonRole>();
            if (restore != null && restore.FriendlyUnits != null && restore.FriendlyUnits.Count > 0)
            {
                states.AddRange(restore.FriendlyUnits);
                foreach (TacticalFriendlyWeaponEntry entry in restore.FriendlyWeapons)
                    if (entry != null && entry.Weapon != null) weapons[entry.UnitId] = entry.Weapon;
                foreach (TacticalUnitState state in states) roles.Add(InferFriendlyRole(state.Id));
            }
            else if (restore != null && restore.UsmcUnit != null)
            {
                // Schema-2 migration: preserve the original platoon exactly.
                states.Add(restore.UsmcUnit);
                weapons[restore.UsmcUnit.Id] = restore.UsmcWeapon;
                roles.Add(TacticalPlatoonRole.Rifle);
            }
            else if (tacticalOperationalEngagement && activeOperationalBattalion != null)
            {
                ...
                foreach (OperationalPlatoonState platoon in activeOperationalBattalion.Platoons)
                {
                    ...
                    if (platoon.Strength < 60) actionPoints = Math.Max(1, actionPoints - TacticalBattalion.ActionPointPenalty);
                    var state = new TacticalUnitState(platoon.Id, platoon.DisplayName, start, actionPoints)
                    {
            ...
            {
                string companyPrefix = activeOperationalBattalion != null ? activeOperationalBattalion.Id : "usmc-company";
                string parentName = activeOperationalBattalion != null ? activeOperationalBattalion.ShortName : "USMC";
                ...
                List<TacticalPlatoonDefinition> roster = activeBattleRequest == null
                    ? TacticalCompany.BuildRoster(tacticalCompanyPreset)
                    : new List<TacticalPlatoonDefinition> { new TacticalPlatoonDefinition("rifle-1", "Rifle Platoon", TacticalPlatoonRole.Rifle) };
                var occupied = new HashSet<HexCoord>();
                HexCoord center = new HexCoord(tacticalBattlefield.Width / 2, tacticalBattlefield.Height / 2);
                HexCoord[] offsets = { new HexCoord(0, 0), new HexCoord(-1, 0), new HexCoord(1, -1), new HexCoord(0, 1) };
                for (int index = 0; index < roster.Count; index++)
                {
                    TacticalPlatoonDefinition definition = roster[index];
                    HexCoord offset = offsets[Math.Min(index, offsets.Length - 1)];
                    HexCoord preferred = new HexCoord(center.Q + offset.Q, center.R + offset.R);
                    HexCoord start = FindOpenLocalDeploymentHex(preferred, occupied);
                    occupied.Add(start);
                    string id = companyPrefix + "-" + definition.IdSuffix;
                    string displayName = parentName + " " + definition.DisplayName;
                    if (index == 0)
                        FindForceImport("usmc-rifle-platoon", id, displayName, out id, out displayName);
                    int roleActionPoints = startingActionPoints + (definition.Role == TacticalPlatoonRole.Reconnaissance ? 2 : definition.Role == TacticalPlatoonRole.Engineers ? 1 : 0);
                    var state = new TacticalUnitState(id, displayName, start, roleActionPoints);
                    TacticalPlatoonCareerRecord veteran = tacticalCareer?.Platoons.Find(record => record.UnitId == id);
                    if (veteran != null)
                    {
                        state.DisplayName = veteran.DisplayName;
                        state.Strength = Math.Min(state.MaximumStrength, Math.Max(40, veteran.Strength + 20));
                        if (veteran.Experience >= 6) state.MaximumActionPoints++;
                        state.RemainingActionPoints = state.MaximumActionPoints;
                    }
                    states.Add(state);
                    roles.Add(definition.Role);
                    weapons[id] = CreateFriendlyWeapon(id, definition.Role);
                }
            }
            ...
            for (int index = 0; index < states.Count; index++)
            {
                ...
                if (role == TacticalPlatoonRole.Engineers && state.MaximumEngineerSupply <= 0)
                {
                    state.MaximumEngineerSupply = TacticalObstacles.DefaultEngineerSupply;
                    state.EngineerSupply = TacticalObstacles.DefaultEngineerSupply;
                }
                if (role == TacticalPlatoonRole.Weapons && state.MaximumMortarRounds <= 0)
                {
                    state.MaximumMortarRounds = TacticalPlannedFires.DefaultMortarAmmunition;
                    state.MortarRounds = TacticalPlannedFires.DefaultMortarAmmunition;
                }
                if (!weapons.TryGetValue(state.Id, out TacticalWeaponState weapon) || weapon == null)
                    weapon = CreateFriendlyWeapon(state.Id, role);
                BuildTacticalFriendlyView(state, weapon, role);
            }

            string selectedId = restore?.SelectedFriendlyUnitId;
            TacticalUnitState selected = tacticalFriendlyStates.Find(candidate => candidate.Id == selectedId) ?? tacticalFriendlyStates[0];
            SetActiveTacticalFriendly(selected.Id, false);
        }

        private void BuildTacticalFriendlyView(TacticalUnitState state, TacticalWeaponState weapon, TacticalPlatoonRole role)
        {
            ...
            UnitCounterView counter = root.AddComponent<UnitCounterView>();
            counter.Initialize(state.DisplayName);
            GameObject illustrated = new GameObject("Illustrated Detail");
            illustrated.transform.SetParent(root.transform, false);
            TacticalFormationView formation = illustrated.AddComponent<TacticalFormationView>();
            string roleLabel = TacticalCompany.ShortRole(role);
            formation.Initialize(TacticalFormationAffiliation.Usmc, "USMC", roleLabel);
            counter.BindRenderers(formation.CommandDeckRenderer, formation.DesignationRenderer);
            GameObject symbolDetail = new GameObject("Symbol Detail");
            symbolDetail.transform.SetParent(root.transform, false);
            NatoSymbolView symbol = symbolDetail.AddComponent<NatoSymbolView>();
            bool supportRole = role != TacticalPlatoonRole.Rifle;
            symbol.Initialize(true, supportRole, "USMC\n" + roleLabel);
            symbol.BindReactive(state, new Color(.22f, .55f, .95f), new Color(.22f, .55f, .95f));
            GameObject miniatureDetail = new GameObject("Miniature Detail");
            miniatureDetail.transform.SetParent(root.transform, false);
            MiniatureCounterView miniature = miniatureDetail.AddComponent<MiniatureCounterView>();
            miniature.Initialize(true, supportRole, "USMC\n" + roleLabel, role);
            miniature.BindReactive(state);
            var view = new TacticalFriendlyView
            {
                State = state,
                Weapon = weapon,
                Role = role,
                ...
                Symbol = symbol,
                Miniature = miniature,
                IllustratedDetail = illustrated,
                SymbolDetail = symbolDetail,
                MiniatureDetail = miniatureDetail
            };
            tacticalFriendlyStates.Add(state);
            tacticalFriendlyViews[state.Id] = view;
            tacticalFriendlyWeapons[state.Id] = weapon;
            tacticalFriendlyByCounter[counter] = view;
            PresentFriendly(view);
            localMovementBoard[state.Position].OccupantId = state.Id;
        }

        private static TacticalWeaponState CreateFriendlyWeapon(string id, TacticalPlatoonRole role)
        {
            string name = role == TacticalPlatoonRole.Weapons ? "Platoon Support Weapons"
                : role == TacticalPlatoonRole.Reconnaissance ? "Recon Platoon Weapons"
                : role == TacticalPlatoonRole.Engineers ? "Engineer Platoon Weapons"
                : "M27 Small Arms";
            return new TacticalWeaponState(id + "-weapon", name, role == TacticalPlatoonRole.Weapons ? 8 : 6);
        }

        private int ActiveReconActionPointCost()
            => InferFriendlyRole(tacticalUnitState?.Id) == TacticalPlatoonRole.Reconnaissance
                ? Math.Max(1, TacticalRecon.ActionPointCost - 1)
                : TacticalRecon.ActionPointCost;

        private static TacticalPlatoonRole InferFriendlyRole(string id)
        {
            if (id != null && id.IndexOf("weapon", StringComparison.OrdinalIgnoreCase) >= 0) return TacticalPlatoonRole.Weapons;
            if (id != null && id.IndexOf("recon", StringComparison.OrdinalIgnoreCase) >= 0) return TacticalPlatoonRole.Reconnaissance;
            if (id != null && (id.IndexOf("engineer", StringComparison.OrdinalIgnoreCase) >= 0 || id.IndexOf("engr", StringComparison.OrdinalIgnoreCase) >= 0)) return TacticalPlatoonRole.Engineers;
            return TacticalPlatoonRole.Rifle;
        }

        private void SetActiveTacticalFriendly(string unitId, bool focusCamera)
        {
            if (!tacticalFriendlyViews.TryGetValue(unitId, out TacticalFriendlyView view)) return;
            if (tacticalUnitState != null) tacticalUnitState.IsSelected = false;
            tacticalUnitState = view.State;
            tacticalWeapon = view.Weapon;
            tacticalUnit = view.Counter;
            tacticalFormationView = view.Formation;
            tacticalSymbolView = view.Symbol;
            tacticalMiniatureView = view.Miniature;
            tacticalIllustratedDetail = view.IllustratedDetail;
            tacticalSymbolDetail = view.SymbolDetail;
            tacticalMiniatureDetail = view.MiniatureDetail;
            if (focusCamera)
            {
                cameraFocus = LocalCounterPosition(view.State.Position);
                ApplyCamera();
            }
            foreach (TacticalFriendlyView friendly in tacticalFriendlyViews.Values) PresentFriendly(friendly);
            AdvanceTutorial(0);
        }

        private void AdvanceTutorial(int completedStep)
        {
            TacticalTutorialState tutorial = tacticalBattlefield?.Tutorial;
            if (tutorial == null || !tutorial.Enabled || tutorial.Completed || tutorial.Step != completedStep) return;
            tutorial.Step++;
            if (tutorial.Step >= 6) tutorial.Completed = true;
        }

        private void PresentFriendly(TacticalFriendlyView view)
        {
            view.Counter.Present(view.State);
            view.Formation.Present(view.State);
            view.IllustratedDetail.SetActive(settings.CounterSkin == CounterSkinTier.Illustrated);
            view.SymbolDetail.SetActive(settings.CounterSkin == CounterSkinTier.Symbol);
            view.MiniatureDetail.SetActive(settings.CounterSkin == CounterSkinTier.Miniature);
        }
            ...
            else if (tacticalOperationalEngagement && tacticalOperationalEnemyBattalion != null)
            {
                ...
                foreach (OperationalPlatoonState platoon in tacticalOperationalEnemyBattalion.Platoons)
                {
                    ...
                    tacticalEnemyWeapons.Add(enemy.Id, new TacticalWeaponState(enemy.Id + "-weapon",
                        support ? "Platoon Support Weapons" : platoon.Role == TacticalPlatoonRole.Reconnaissance ? "Recon Platoon Weapons" : "Platoon Small Arms",
                        support ? 8 : 6));
                    enemyMarkerLabels.Add(TacticalCompany.ShortRole(platoon.Role));
                }
            }
            else
            {
                ...
                for (int slot = 0; slot < roster.Count; slot++)
                {
                    ...
                    string displayName = role == TacticalScenario.SupportRole ? $"PLANMC Weapons Platoon {perRoleIndex}" : $"PLANMC Rifle Platoon {perRoleIndex}";
                    var enemy = new TacticalUnitState(id, displayName, position, 4);
                    tacticalEnemyStates.Add(enemy);
                    tacticalEnemyWeapons.Add(enemy.Id, new TacticalWeaponState(enemy.Id + "-weapon", role == TacticalScenario.SupportRole ? "Platoon Support Weapons" : "Platoon Small Arms", role == TacticalScenario.SupportRole ? 8 : 6));
                    enemyMarkerLabels.Add(role == TacticalScenario.SupportRole ? "WPNS PLT" : "RIFLE PLT");
                }
            }
            for (int index = 0; index < tacticalEnemyStates.Count; index++)
            {
        ...
        private HashSet<HexCoord> FriendlyPositions()
        {
            var positions = new HashSet<HexCoord>();
            foreach (TacticalUnitState friendly in tacticalFriendlyStates) positions.Add(friendly.Position);
            return positions;
        }
        ...
        {
            if (tacticalFriendlyStates.Count == 0) return;
            foreach (KeyValuePair<HexCoord, HexCellView> pair in localCells)
            {
                float bestFog = .66f;
                foreach (TacticalUnitState friendly in tacticalFriendlyStates)
                {
                    TacticalLosResult los = TacticalLineOfSight.Inspect(localMovementBoard, friendly.Position, pair.Key);
                    float fog;
                    if (pair.Key.Equals(friendly.Position)) fog = 0f;
                    ...
                    bestFog = Mathf.Min(bestFog, fog);
                }
                pair.Value.SetFog(bestFog);
            }

            foreach (TacticalUnitState enemy in tacticalEnemyStates)
            {
                tacticalContacts.TryGetValue(enemy.Id, out TacticalContactState previous);
                TacticalContactState report = BestFriendlyContactOnEnemy(enemy, previous);
                ApplyIlluminationBonus(report, enemy.Position);
            ...
            foreach (TacticalUnitState enemy in tacticalEnemyStates)
            {
                ...
                TacticalUnitState target = FindBestTargetForEnemy(enemy, previousEnemyContact, out TacticalContactState enemyContact);
        ...
        private TacticalContactState BestFriendlyContactOnEnemy(TacticalUnitState enemy, TacticalContactState previous)
        {
            TacticalContactState best = null;
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
            {
                TacticalContactState candidate = TacticalObservation.Check(localMovementBoard, friendly.Id, friendly.Position,
                    enemy.Id, enemy.DisplayName, enemy.Position, turnState.TurnNumber,
                    previous != null && previous.ObserverId == friendly.Id ? previous : null, enemy);
                ApplyVisibilityConditions(candidate);
                ApplyIlluminationBonus(candidate, enemy.Position);
                if (best == null || candidate.State > best.State || candidate.State == best.State && candidate.RangeHexes < best.RangeHexes)
                    best = candidate;
            }
            if (best != null && best.State == TacticalVisibilityState.Hidden && previous != null && previous.State != TacticalVisibilityState.Hidden)
                best = TacticalObservation.Check(localMovementBoard, previous.ObserverId, FindFriendly(previous.ObserverId)?.Position ?? tacticalUnitState.Position,
                    enemy.Id, enemy.DisplayName, enemy.Position, turnState.TurnNumber, previous, enemy);
            return best;
        }

        private TacticalUnitState FindBestTargetForEnemy(TacticalUnitState enemy, TacticalContactState previous, out TacticalContactState bestContact)
        {
            TacticalUnitState bestTarget = tacticalFriendlyStates[0];
            bestContact = null;
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
            {
                TacticalContactState candidate = TacticalObservation.Check(localMovementBoard, enemy.Id, enemy.Position,
                    friendly.Id, friendly.DisplayName, friendly.Position, turnState.TurnNumber,
                    previous != null && previous.TargetId == friendly.Id ? previous : null, friendly);
                ApplyVisibilityConditions(candidate);
                candidate.State = TacticalRecon.ApplyBonus(candidate.State, TacticalRecon.IsUnderActiveRecon(tacticalPlaReconMarkers, friendly.Position));
                if (bestContact == null || candidate.State > bestContact.State || candidate.State == bestContact.State && candidate.RangeHexes < bestContact.RangeHexes)
                {
        ...
        private void ApplyVisibilityConditions(TacticalContactState contact)
        {
            if (contact == null || tacticalBattlefield?.QuickBattle == null || activeBattleRequest != null) return;
            int effectiveRange = TacticalStandaloneRules.ObservationRange(tacticalBattlefield.QuickBattle, TacticalObservation.ClearObservationRangeHexes);
            if (contact.RangeHexes <= effectiveRange || contact.State == TacticalVisibilityState.Hidden) return;
            contact.State = (TacticalVisibilityState)Math.Max((int)TacticalVisibilityState.Contact, (int)contact.State - 1);
        }

        private void ApplyIlluminationBonus(TacticalContactState contact, HexCoord position)
        {
            if (contact == null || contact.LineOfSight == TacticalLosState.Blocked ||
                !TacticalPlannedFires.IsIlluminated(tacticalBattlefield?.ActiveIlluminationMarkers, position, turnState.TurnNumber)) return;
            contact.State = TacticalVisibilityState.Observed;
            contact.IsStale = false;
            contact.LastKnownPosition = position;
            contact.LastObservedTurn = turnState.TurnNumber;
        }

        private TacticalUnitState FindFriendly(string id)
            => tacticalFriendlyStates.Find(candidate => candidate.Id == id);

        private bool IsFriendlyId(string id)
            => FindFriendly(id) != null;

        private bool ObjectiveControlledByUsmc()
            => tacticalObjective != null && localMovementBoard.TryGetValue(tacticalObjective.ObjectiveHex, out TacticalMovementCell cell) && IsFriendlyId(cell.OccupantId);

        private TacticalCombatStatus WorstFriendlyStatus()
        {
            TacticalCombatStatus worst = TacticalCombatStatus.Ready;
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
                if (friendly.CombatStatus > worst) worst = friendly.CombatStatus;
            return worst;
        }
        ...
        private void FrameObservationContacts()
        {
            ...
            int count = 0;
            foreach (TacticalFriendlyView friendly in tacticalFriendlyViews.Values)
            {
                total += friendly.Counter.transform.position;
                count++;
            }
        ...
        private void UpdateTacticalPointer()
        {
            ...
            if (tacticalUnitMoving || tacticalFireResolving || tacticalEnemyTurnActive || resultScreenActive || campaignBriefingActive || settingsPanelOpen || supportCardModalActive || tacticalForcePanelOpen) return;
            if (Input.GetKeyDown(KeyCode.N))
            {
                CycleTacticalFriendly(false);
                return;
            }
            if (Input.GetKeyDown(KeyCode.Tab))
            {
                CycleTacticalFriendly(true);
                return;
            }
            Vector2 guiPointer = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y) / GetUiScale();
            if (tacticalMenuOpen && tacticalMenuRect.Contains(guiPointer) ||
                !hudCollapsed && (returnToIslandRect.Contains(guiPointer) || new Rect(20f, 18f, 405f, 318f).Contains(guiPointer))) return;
            ...
            TacticalFriendlyView pointedFriendly = null;
            foreach (RaycastHit hit in Physics.RaycastAll(ray, 100f))
            {
                ...
                if (candidate != null && tacticalFriendlyByCounter.TryGetValue(candidate, out TacticalFriendlyView friendly))
                    pointedFriendly = friendly;
            }
            if (Input.GetMouseButtonDown(1))
            {
                if (tacticalMovePlanning || tacticalLosPlanning || tacticalFirePlanning || tacticalReconPlanning || tacticalSpecialPlanning != TacticalSpecialOrder.None)
                {
                    CancelTacticalInteraction(true);
                    return;
                }
                if (pointedFriendly != null)
                {
                    SetActiveTacticalFriendly(pointedFriendly.State.Id, false);
            ...
            if (Input.GetMouseButtonDown(0) && tacticalSpecialPlanning != TacticalSpecialOrder.None && next != null)
            {
                TryIssueTacticalSpecialOrder(next.Coord);
                return;
            }
            if (Input.GetMouseButtonDown(0) && pointedFriendly != null &&
                !tacticalMovePlanning && !tacticalFirePlanning && !tacticalReconPlanning && !tacticalLosPlanning && tacticalSpecialPlanning == TacticalSpecialOrder.None)
            {
                CancelTacticalInteraction(false);
                SetActiveTacticalFriendly(pointedFriendly.State.Id, false);
                tacticalOrderFeedback = pointedFriendly.State.DisplayName.ToUpperInvariant() + " SELECTED";
            ...
            PreviewTacticalSpecialOrder(hoveredLocalCell);
        }

        private void CycleTacticalFriendly(bool actionableOnly)
        {
            if (tacticalFriendlyStates.Count == 0) return;
            CancelTacticalInteraction(false);
            int start = Math.Max(0, tacticalFriendlyStates.FindIndex(candidate => candidate.Id == tacticalUnitState?.Id));
            for (int offset = 1; offset <= tacticalFriendlyStates.Count; offset++)
            {
                TacticalUnitState candidate = tacticalFriendlyStates[(start + offset) % tacticalFriendlyStates.Count];
                if (actionableOnly && !candidate.CanMove && !candidate.CanRally) continue;
                SetActiveTacticalFriendly(candidate.Id, true);
                tacticalOrderFeedback = candidate.DisplayName.ToUpperInvariant() + " SELECTED";
                tacticalAudio.Play(TacticalSound.Select);
                return;
            }
            tacticalOrderFeedback = "NO ACTIONABLE PLATOONS";
            tacticalAudio.Play(TacticalSound.OrderCancel);
        }

        private void UpdateTacticalDeploymentPointer()
        {
            if (!Input.GetMouseButtonDown(0)) return;
            Vector2 guiPointer = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y) / GetUiScale();
            float uiWidth = Screen.width / GetUiScale();
            if (new Rect(uiWidth / 2f - 260f, 18f, 520f, 132f).Contains(guiPointer)) return;
            Ray ray = mapCamera.ScreenPointToRay(Input.mousePosition);
            if (tacticalObstaclePlacementActive)
            {
                HexCellView obstacleHex = PickLocalHexAtScreenPoint(Input.mousePosition);
                if (obstacleHex == null) return;
                TacticalMovementCell obstacleCell = null;
                bool legalObstacle = obstacleHex.IsLand && tacticalDefensiveObstacleBudget > 0 &&
                    HexCoord.Distance(tacticalObjective.ObjectiveHex, obstacleHex.Coord) <= 4 &&
                    FindTacticalObstacle(obstacleHex.Coord) == null &&
                    localMovementBoard.TryGetValue(obstacleHex.Coord, out obstacleCell) &&
                    string.IsNullOrEmpty(obstacleCell.OccupantId);
                if (!legalObstacle)
                {
                    obstacleHex.SetInvalid(true);
                    tacticalOrderFeedback = "DEFENSE REJECTED • USE OPEN GROUND WITHIN 4 HEXES OF OBJECTIVE";
                    tacticalAudio.Play(TacticalSound.OrderCancel);
                    return;
                }
                var obstacle = new TacticalObstacleState
                {
                    Id = $"USMC-OBS-{tacticalBattlefield.Obstacles.Count + 1:D2}-{obstacleHex.Coord.Q:D2}-{obstacleHex.Coord.R:D2}",
                    Position = obstacleHex.Coord,
                    Type = tacticalDefensiveObstacleType,
                    OwnerSide = "USMC",
                    UsmcIntelligence = TacticalObstacleIntelligence.Identified
                };
                tacticalBattlefield.Obstacles.Add(obstacle);
                TacticalObstacles.ApplyToCell(obstacleCell, obstacle);
                RefreshTacticalObstacleView(obstacle);
                tacticalDefensiveObstacleBudget--;
                if (tacticalDefensiveObstacleBudget <= 0) tacticalObstaclePlacementActive = false;
                RecordTacticalObstacleEvent(TacticalObstacleEventKind.Marked, null, obstacle, 0, 0,
                    $"USMC placed {obstacle.Type} before battle");
                tacticalOrderFeedback = tacticalDefensiveObstacleBudget > 0
                    ? $"{obstacle.Type.ToString().ToUpperInvariant()} PLACED • {tacticalDefensiveObstacleBudget} DEFENSES REMAIN"
                    : "DEFENSIVE OBSTACLES COMPLETE";
                tacticalAudio.Play(TacticalSound.OrderConfirm);
                return;
            }
            foreach (RaycastHit hit in Physics.RaycastAll(ray, 100f))
            {
                UnitCounterView counter = hit.collider.GetComponentInParent<UnitCounterView>();
        ...
        private void BeginTacticalDeployment(TacticalCompanyPreset preset)
        {
            foreach (TacticalFriendlyView friendly in tacticalFriendlyViews.Values)
            {
                if (localMovementBoard.TryGetValue(friendly.State.Position, out TacticalMovementCell cell) && cell.OccupantId == friendly.State.Id)
                    cell.OccupantId = null;
                if (friendly.Counter != null) Destroy(friendly.Counter.gameObject);
            }
            tacticalCompanyPreset = preset;
            tacticalBattlefield.QuickBattle.CompanyPreset = preset;
            ApplyQuickBattleConfiguration();
            BuildTacticalUnits();
            tacticalForcePanelOpen = false;
            tacticalDeploymentActive = true;
            tacticalObstaclePlacementActive = false;
            tacticalDefensiveObstacleBudget = tacticalObjective.MissionType == TacticalMissionType.Defend
                ? TacticalObstacles.DefaultDefensiveObstacleBudget : 0;
            tacticalDeploymentUnit = tacticalUnitState;
            ClearTacticalReachable();
            HexCoord center = new HexCoord(tacticalBattlefield.Width / 2, tacticalBattlefield.Height / 2);
            foreach (KeyValuePair<HexCoord, HexCellView> pair in localCells)
            {
                if (!pair.Value.IsLand || HexCoord.Distance(center, pair.Key) > 3) continue;
                tacticalReachable[pair.Key] = 0;
                pair.Value.SetReachable(true);
            }
            tacticalOrderFeedback = "DEPLOYMENT • SELECT A PLATOON, THEN AN OPEN TEAL HEX";
        }

        private void ApplyQuickBattleConfiguration()
        {
            TacticalQuickBattleConfiguration config = tacticalBattlefield.QuickBattle;
            if (config == null || activeBattleRequest != null) return;
            tacticalObjective.MissionType = config.MissionType;
            tacticalObjective.Posture = TacticalVictory.SitingShapeFor(config.MissionType);
            tacticalObjective.TurnLimit = Math.Max(4, config.TurnLimit);
            tacticalBattlefield.SecondaryObjectives = TacticalStandaloneRules.CreateSecondaryObjectives(
                config.MissionType, tacticalCompanyPreset == TacticalCompanyPreset.Assault);
            tacticalBattlefield.Tutorial.Enabled = config.TutorialEnabled;
            tacticalBattlefield.Tutorial.Step = 0;
            tacticalBattlefield.Tutorial.Completed = false;
            int enemyStrength = TacticalStandaloneRules.EnemyStrength(config.Difficulty);
            foreach (TacticalUnitState enemy in tacticalEnemyStates)
            {
                enemy.MaximumStrength = enemyStrength;
                enemy.Strength = enemyStrength;
                if (config.Difficulty == TacticalDifficulty.Elite) enemy.MaximumActionPoints++;
                enemy.RemainingActionPoints = enemy.MaximumActionPoints;
            }
        }

        private void CycleQuickMission()
        {
            TacticalQuickBattleConfiguration config = tacticalBattlefield.QuickBattle;
            config.MissionType = (TacticalMissionType)(((int)config.MissionType + 1) % Enum.GetValues(typeof(TacticalMissionType)).Length);
        }

        private void CycleQuickWeather()
            => tacticalBattlefield.QuickBattle.Weather = (TacticalWeather)(((int)tacticalBattlefield.QuickBattle.Weather + 1) % 3);

        private void CycleQuickVisibility()
            => tacticalBattlefield.QuickBattle.Visibility = (TacticalVisibility)(((int)tacticalBattlefield.QuickBattle.Visibility + 1) % 3);

        private void CycleQuickDifficulty()
            => tacticalBattlefield.QuickBattle.Difficulty = (TacticalDifficulty)(((int)tacticalBattlefield.QuickBattle.Difficulty + 1) % 3);

        private void ConfirmTacticalDeployment()
        {
            tacticalDeploymentActive = false;
            tacticalObstaclePlacementActive = false;
            tacticalDeploymentUnit = null;
            ...
            tacticalOrderFeedback = "COMPANY DEPLOYED • SELECT A PLATOON";
        }
        ...
        private void OpenTacticalMenu(Vector2 pointer)
        {
            ...
            tacticalMenuRect = new Rect(
                Mathf.Clamp(pointer.x, 8f, Screen.width / GetUiScale() - 358f),
        ...
        private void BeginTacticalMovePlanning()
        {
            ...
            foreach (KeyValuePair<HexCoord, int> pair in TacticalMovementPlanner.Reachable(
                         localMovementBoard, tacticalUnitState.Position,
                         TacticalFireAndManeuver.PlanningBudget(tacticalUnitState.RemainingActionPoints, tacticalUnitState.MovementPosture), tacticalUnitState.Id))
            {
        ...
        private void PreviewTacticalFire(HexCellView targetCell)
        {
            ...
            if (tacticalFirePreview.IsValid)
            {
                ...
                int positionModifier = TacticalCombatPower.EntrenchmentHitModifier(tacticalFireTarget, targetCell.Coord, tacticalUnitState.Position);
                if (positionModifier != 0)
                {
                    int fullPositionModifier = TacticalCombatPower.EntrenchmentHitModifier(tacticalFireTarget, targetCell.Coord);
                    string positionAspect = positionModifier == fullPositionModifier ? "front" : "flank";
                    tacticalFirePreview.Modifiers.Add(new TacticalFireModifier { Label = $"Fighting position {positionAspect} L{tacticalFireTarget.EntrenchmentLevel}", Value = positionModifier });
                ...
                ApplyBattleConditionsModifier(tacticalFirePreview);
                TacticalConcealmentMorale.ApplyAmbushModifier(tacticalFirePreview, tacticalUnitState);
            }
        ...
        private bool TryIssueTacticalFire(HexCoord targetPosition)
        {
            ...
            bool wasAmbush = tacticalUnitState.IsConcealed && tacticalUnitState.AmbushReady;
            tacticalUnitState.FiredThisTurn = true;
            if (TacticalConcealmentMorale.Reveal(tacticalUnitState))
                RecordConcealmentEvent(wasAmbush ? TacticalConcealmentEventKind.Ambush : TacticalConcealmentEventKind.Reveal,
                    tacticalUnitState, target, tacticalUnitState.Position, wasAmbush ? "Prepared ambush fired; position revealed" : "Firing revealed position");
            if (target != null) target.WasFiredUponThisTurn = true;
            ...
            ResolvePendingFallback(target, tacticalFriendlyStates);
            tacticalFirePlanning = false;
        ...
        private static void ApplyDefensivePositionModifier(TacticalFirePreview preview, TacticalUnitState target)
        {
            ...
            int modifier = TacticalCombatPower.EntrenchmentHitModifier(target, preview.TargetPosition, preview.AttackerPosition);
            if (modifier == 0) return;
            int fullModifier = TacticalCombatPower.EntrenchmentHitModifier(target, preview.TargetPosition);
            string aspect = modifier == fullModifier ? "front" : "flank";
            preview.Modifiers.Add(new TacticalFireModifier { Label = $"Fighting position {aspect} L{target.EntrenchmentLevel}", Value = modifier });
        ...
        private void ApplyBattleConditionsModifier(TacticalFirePreview preview)
        {
            if (preview == null || !preview.IsValid || tacticalBattlefield?.QuickBattle == null || activeBattleRequest != null) return;
            int modifier = TacticalStandaloneRules.FireModifier(tacticalBattlefield.QuickBattle);
            if (modifier == 0) return;
            preview.Modifiers.Add(new TacticalFireModifier
            {
                Label = tacticalBattlefield.QuickBattle.Weather + " / " + tacticalBattlefield.QuickBattle.Visibility,
                Value = modifier
            });
            preview.HitChance = Math.Max(5, Math.Min(90, preview.HitChance + modifier));
            preview.SuppressionChance = Math.Max(5, Math.Min(95, preview.SuppressionChance + modifier / 2));
        ...
        {
            int reconCost = ActiveReconActionPointCost();
            if (tacticalUnitState.RemainingActionPoints < reconCost) return;
            tacticalMenuOpen = false;
            tacticalMovePlanning = false;
            tacticalLosPlanning = false;
            tacticalFirePlanning = false;
            ...
            tacticalAudio.Play(TacticalSound.Inspect);
            AdvanceTutorial(3);
            tacticalOrderFeedback = $"RECON • {reconCost} AP • {TacticalRecon.MaximumRangeHexes} hex max • Hover a hex";
            PreviewTacticalRecon(hoveredLocalCell);
        }
        ...
        private bool TryIssueTacticalRecon(HexCoord target)
        {
            ...
            if (!tacticalUnitState.TrySpendActionPoints(ActiveReconActionPointCost()))
            {
        ...
        private void CycleMovementPosture()
        {
            tacticalUnitState.MovementPosture = (TacticalMovementPosture)(((int)tacticalUnitState.MovementPosture + 1) % 3);
            tacticalOrderFeedback = $"MOVEMENT POSTURE • {tacticalUnitState.MovementPosture.ToString().ToUpperInvariant()}";
            tacticalAudio.Play(TacticalSound.Select);
            AdvanceTutorial(1);
        }

        private void CycleReactionPolicy()
        {
            tacticalUnitState.ReactionPolicy = (TacticalReactionPolicy)(((int)tacticalUnitState.ReactionPolicy + 1) % 3);
            tacticalOrderFeedback = $"REACTION POLICY • {tacticalUnitState.ReactionPolicy.ToString().ToUpperInvariant()}";
            tacticalAudio.Play(TacticalSound.Select);
        }
        ...
        private void IssueEntrench()
        {
            ...
            tacticalBattlefield.PositionEvents.Add(new TacticalPositionEvent
            {
                ...
                FacingSector = tacticalUnitState.EntrenchmentFacingSector
            });
        ...
        private void IssueResupply()
        {
            ...
            int engineerSupplyBefore = tacticalUnitState.EngineerSupply;
            ...
            if (tacticalUnitState.MaximumEngineerSupply > 0)
                tacticalUnitState.EngineerSupply = Math.Min(tacticalUnitState.MaximumEngineerSupply, tacticalUnitState.EngineerSupply + 2);
            tacticalBattlefield.LogisticsEvents.Add(new TacticalLogisticsEvent
            {
                Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId, Turn = turnState.TurnNumber,
                UnitId = tacticalUnitState.Id, SourceUnitId = source.Id,
                Kind = tacticalUnitState.MaximumEngineerSupply > 0 ? "Resupply + engineer stores" : "Resupply",
            ...
            string engineerFeedback = tacticalUnitState.MaximumEngineerSupply > 0
                ? $" • ENG {engineerSupplyBefore}→{tacticalUnitState.EngineerSupply}" : string.Empty;
            FinishTacticalSpecialOrder($"RESUPPLIED • AMMO {ammoBefore}→{tacticalWeapon.RemainingAmmunition} • SUPPLY {supplyBefore}→{tacticalUnitState.Supply}{engineerFeedback}");
        }

        private void IssueHide()
        {
            if (!localMovementBoard.TryGetValue(tacticalUnitState.Position, out TacticalMovementCell cell) ||
                !TacticalConcealmentMorale.TryHide(tacticalUnitState, cell, turnState.TurnNumber))
            {
                tacticalOrderFeedback = "HIDE REQUIRES 2 AP, NO MOVE/FIRE, AND MEDIUM OR HEAVY COVER";
                tacticalAudio.Play(TacticalSound.OrderCancel);
                return;
            }
            RecordConcealmentEvent(TacticalConcealmentEventKind.Hide, tacticalUnitState, null,
                tacticalUnitState.Position, "Concealed in covered terrain; ambush prepared");
            tacticalMenuOpen = false;
            PresentFriendly(tacticalFriendlyViews[tacticalUnitState.Id]);
            RefreshTacticalObservation();
            tacticalOrderFeedback = "CONCEALED - AMBUSH READY";
            tacticalAudio.Play(TacticalSound.OrderConfirm);
        }

        private TacticalConcealmentEvent RecordConcealmentEvent(TacticalConcealmentEventKind kind,
            TacticalUnitState unit, TacticalUnitState target, HexCoord destination, string summary, int actionPointCost = 0,
            HexCoord? originOverride = null)
        {
            if (unit == null || tacticalBattlefield == null) return null;
            var entry = new TacticalConcealmentEvent
            {
                Sequence = ++tacticalEventSequence,
                BattlefieldId = tacticalBattlefield.BattlefieldId,
                Turn = turnState.TurnNumber,
                ...
                UnitId = unit.Id,
                TargetId = target?.Id,
                Origin = originOverride ?? unit.Position,
                Destination = destination,
                ActionPointCost = actionPointCost,
                Summary = summary
            };
            if (tacticalBattlefield.ConcealmentEvents == null)
                tacticalBattlefield.ConcealmentEvents = new List<TacticalConcealmentEvent>();
            tacticalBattlefield.ConcealmentEvents.Add(entry);
            Debug.Log($"ALWAYS_FAITHFUL_CONCEALMENT_EVENT sequence={entry.Sequence} kind={entry.Kind} unit={entry.UnitId} target={entry.TargetId} destination={entry.Destination}");
            return entry;
        }

        private void ResolvePendingFallback(TacticalUnitState unit, IEnumerable<TacticalUnitState> threats)
        {
            if (unit == null || unit.MoraleState != TacticalMoraleState.FallingBack) return;
            HexCoord origin = unit.Position;
            if (TacticalConcealmentMorale.TryChooseFallbackHex(localMovementBoard, unit, threats, out HexCoord destination))
            {
                localMovementBoard[origin].OccupantId = null;
                localMovementBoard[destination].OccupantId = unit.Id;
                ...
                unit.MoraleState = TacticalMoraleState.Routed;
                RecordConcealmentEvent(TacticalConcealmentEventKind.FallBack, unit, null, destination,
                    $"Forced fallback from {origin} to {destination}", 0, origin);
                if (tacticalFriendlyViews.TryGetValue(unit.Id, out TacticalFriendlyView friendlyView))
                {
                    friendlyView.Counter.transform.position = LocalCounterPosition(destination);
                    PresentFriendly(friendlyView);
                }
                if (tacticalContactViews.TryGetValue(unit.Id, out ContactMarkerView enemyView))
                    enemyView.transform.position = LocalCounterPosition(destination) + Vector3.up * .03f;
                return;
            }

            unit.RemainingActionPoints = 0;
            unit.ReactionPoints = 0;
            unit.MoraleState = TacticalConcealmentMorale.IsAdjacentToThreat(unit, threats)
                ? TacticalMoraleState.Surrendered
                : TacticalMoraleState.Routed;
            RecordConcealmentEvent(unit.MoraleState == TacticalMoraleState.Surrendered
                    ? TacticalConcealmentEventKind.Surrender : TacticalConcealmentEventKind.Rout,
                unit, null, origin, unit.MoraleState == TacticalMoraleState.Surrendered
                    ? "No safe retreat while adjacent to the enemy; platoon surrendered"
                    : "No safe fallback hex; platoon routed in place");
        }

        private void BeginTacticalSpecialOrder(TacticalSpecialOrder order)
        {
            tacticalMenuOpen = false;
            tacticalMovePlanning = tacticalLosPlanning = tacticalFirePlanning = tacticalReconPlanning = false;
            tacticalSpecialPlanning = order;
            tacticalUnitState.IsSelected = true;
            tacticalUnit.Present(tacticalUnitState);
            tacticalOrderFeedback = order == TacticalSpecialOrder.Face ? "FACE • Choose a direction"
                ...
                : order == TacticalSpecialOrder.MortarFire ? $"MORTAR {FireMissionShortLabel(SelectedMortarMissionType())} • {TacticalPlannedFires.MortarFireActionPointCost} AP • {tacticalUnitState.MortarRounds} ROUNDS"
                : $"PLANNED {FireMissionShortLabel(tacticalFireMissionType)} • {TacticalPlannedFires.OrderActionPointCost} AP • IMPACT NEXT TURN";
            if (order == TacticalSpecialOrder.Search)
                tacticalOrderFeedback = $"SEARCH / CLEAR - adjacent hex - {TacticalConcealmentMorale.SearchActionPointCost} AP";
            else if (order == TacticalSpecialOrder.Withdraw)
                tacticalOrderFeedback = $"WITHDRAW - adjacent hex away from enemy - {TacticalConcealmentMorale.WithdrawActionPointCost} AP";
            else if (order == TacticalSpecialOrder.DetectObstacle)
                tacticalOrderFeedback = $"DETECT / MARK - adjacent hex - {TacticalObstacles.DetectActionPointCost} AP";
            else if (order == TacticalSpecialOrder.HastyBreach)
                tacticalOrderFeedback = $"HASTY BREACH - {TacticalObstacles.HastyBreachActionPointCost} AP / {TacticalObstacles.HastyBreachSupplyCost} engineer supply";
            else if (order == TacticalSpecialOrder.DeliberateBreach)
                tacticalOrderFeedback = $"DELIBERATE BREACH - {TacticalObstacles.DeliberateBreachActionPointCost} AP / {TacticalObstacles.DeliberateBreachSupplyCost} engineer supply";
            tacticalAudio.Play(TacticalSound.Inspect);
        }

        private void PreviewTacticalSpecialOrder(HexCellView cell)
        {
            if (tacticalSpecialPlanning == TacticalSpecialOrder.None || cell == null) return;
            int range = HexCoord.Distance(tacticalUnitState.Position, cell.Coord);
            bool legal = tacticalSpecialPlanning == TacticalSpecialOrder.Face ? range > 0
                ...
                : tacticalSpecialPlanning == TacticalSpecialOrder.MortarFire
                    ? TacticalPlannedFires.CanOrder(tacticalUnitState, tacticalBattlefield.FireSupport, cell.Coord,
                        turnState.TurnNumber, true, out _)
                    : range <= TacticalPlannedFires.MaximumRangeHexes && cell.IsLand;
            if (tacticalSpecialPlanning == TacticalSpecialOrder.Search)
                legal = range == TacticalConcealmentMorale.SearchRangeHexes && cell.IsLand;
            else if (tacticalSpecialPlanning == TacticalSpecialOrder.Withdraw)
                legal = TacticalConcealmentMorale.CanWithdraw(tacticalUnitState, cell.Coord, localMovementBoard, tacticalEnemyStates);
            else if (tacticalSpecialPlanning == TacticalSpecialOrder.DetectObstacle)
                legal = range <= TacticalObstacles.EngineerRangeHexes && cell.IsLand;
            else if (tacticalSpecialPlanning == TacticalSpecialOrder.HastyBreach || tacticalSpecialPlanning == TacticalSpecialOrder.DeliberateBreach)
                legal = TacticalObstacles.CanBreach(tacticalUnitState, FindTacticalObstacle(cell.Coord),
                    InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Engineers,
                    tacticalSpecialPlanning == TacticalSpecialOrder.DeliberateBreach);
        ...
        private bool TryIssueTacticalSpecialOrder(HexCoord target)
        {
            TacticalSpecialOrder order = tacticalSpecialPlanning;
            int range = HexCoord.Distance(tacticalUnitState.Position, target);
            ...
            if (order == TacticalSpecialOrder.DetectObstacle)
            {
                TacticalObstacleState obstacle = FindTacticalObstacle(target);
                if (!TacticalObstacles.TryDetect(tacticalUnitState, obstacle, target,
                        InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Engineers))
                    return RejectTacticalSpecialOrder(target, "DETECT / MARK REQUIRES AN ENGINEER, ADJACENT HEX, AND 2 AP");
                if (obstacle != null)
                {
                    TacticalObstacles.ApplyToCell(cell, obstacle);
                    RefreshTacticalObstacleView(obstacle);
                    RecordTacticalObstacleEvent(TacticalObstacleEventKind.Marked, tacticalUnitState, obstacle, 0, 0,
                        $"Engineers identified and marked {obstacle.Type}", TacticalObstacles.DetectActionPointCost);
                }
                FinishTacticalSpecialOrder(obstacle == null ? "ENGINEER SWEEP COMPLETE - CLEAR" : $"{obstacle.Type.ToString().ToUpperInvariant()} IDENTIFIED AND MARKED");
                return true;
            }
            if (order == TacticalSpecialOrder.HastyBreach || order == TacticalSpecialOrder.DeliberateBreach)
            {
                bool deliberate = order == TacticalSpecialOrder.DeliberateBreach;
                TacticalObstacleState obstacle = FindTacticalObstacle(target);
                int supplyBefore = tacticalUnitState.EngineerSupply;
                int seed = TacticalDirectFire.CreateSeed(tacticalBattlefield.BattlefieldId, turnState.TurnNumber, tacticalEventSequence + 1);
                if (!TacticalObstacles.TryBreach(tacticalUnitState, obstacle,
                        InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Engineers, deliberate, seed, out bool succeeded))
                    return RejectTacticalSpecialOrder(target, "BREACH REQUIRES A MARKED ADJACENT OBSTACLE, AP, AND ENGINEER SUPPLY");
                TacticalObstacles.ApplyToCell(cell, obstacle);
                RefreshTacticalObstacleView(obstacle);
                int apCost = deliberate ? TacticalObstacles.DeliberateBreachActionPointCost : TacticalObstacles.HastyBreachActionPointCost;
                RecordTacticalObstacleEvent(deliberate ? TacticalObstacleEventKind.DeliberateBreach : TacticalObstacleEventKind.HastyBreach,
                    tacticalUnitState, obstacle, succeeded ? 0 : 5, 0,
                    succeeded ? $"{(deliberate ? "Deliberate" : "Hasty")} breach opened a safe lane" : "Hasty breach failed; engineers suppressed",
                    apCost, supplyBefore, tacticalUnitState.EngineerSupply, succeeded);
                if (succeeded)
                    RecordTacticalObstacleEvent(TacticalObstacleEventKind.Breached, tacticalUnitState, obstacle, 0, 0,
                        "Persistent safe lane established", apCost, supplyBefore, tacticalUnitState.EngineerSupply);
                FinishTacticalSpecialOrder(succeeded ? "BREACH COMPLETE - SAFE LANE OPEN" : "HASTY BREACH FAILED - ENGINEERS SUPPRESSED");
                return true;
            }
            if (order == TacticalSpecialOrder.Search)
            {
                if (!TacticalConcealmentMorale.TrySearch(tacticalUnitState, target))
                    return RejectTacticalSpecialOrder(target, "SEARCH REQUIRES AN ADJACENT HEX AND 2 AP");
                TacticalUnitState discovered = FindEnemyAt(target);
                bool revealed = discovered != null && TacticalConcealmentMorale.Reveal(discovered);
                RecordConcealmentEvent(TacticalConcealmentEventKind.Search, tacticalUnitState, discovered, target,
                    revealed ? $"{discovered.DisplayName} exposed by search" : "Covered position searched; no concealed formation found",
                    TacticalConcealmentMorale.SearchActionPointCost);
                RefreshTacticalObservation();
                FinishTacticalSpecialOrder(revealed ? "SEARCH COMPLETE - ENEMY EXPOSED" : "SEARCH COMPLETE - CLEAR");
                return true;
            }
            if (order == TacticalSpecialOrder.Withdraw)
            {
                HexCoord origin = tacticalUnitState.Position;
                if (!TacticalConcealmentMorale.TryWithdraw(tacticalUnitState, target, localMovementBoard, tacticalEnemyStates))
                    return RejectTacticalSpecialOrder(target, "WITHDRAW MUST MOVE ONE HEX FARTHER FROM THE NEAREST ENEMY");
                localMovementBoard[origin].OccupantId = null;
                localMovementBoard[target].OccupantId = tacticalUnitState.Id;
                tacticalUnitState.Position = target;
                tacticalUnit.transform.position = LocalCounterPosition(target);
                RecordConcealmentEvent(TacticalConcealmentEventKind.Withdraw, tacticalUnitState, null, target,
                    $"Controlled withdrawal from {origin} to {target}", TacticalConcealmentMorale.WithdrawActionPointCost, origin);
                RefreshTacticalObservation();
                FinishTacticalSpecialOrder("WITHDRAWAL COMPLETE - COHESION IMPROVED");
                return true;
            }
            if (order == TacticalSpecialOrder.CallForFire)
            {
                return OrderPlannedFire(target, tacticalFireMissionType);
            }
            if (order == TacticalSpecialOrder.MortarFire)
            {
                return OrderPlannedFire(target, SelectedMortarMissionType());
            }
            if (order == TacticalSpecialOrder.Smoke)
            {
                if (range > TacticalFireAndManeuver.SmokeRangeHexes || !tacticalUnitState.TrySpendActionPoints(TacticalFireAndManeuver.SmokeActionPointCost))
                    return RejectTacticalSpecialOrder(target, "SMOKE OUT OF RANGE OR INSUFFICIENT AP");
                int expires = turnState.TurnNumber + TacticalFireAndManeuver.SmokeDurationTurns - 1;
                cell.SmokeExpiresAfterTurn = expires;
                tacticalBattlefield.ActiveSmokeMarkers.RemoveAll(marker => marker.Position.Equals(target));
                tacticalBattlefield.ActiveSmokeMarkers.Add(new TacticalSmokeMarker { Position = target, PlacedTurn = turnState.TurnNumber, ExpiresAfterTurn = expires, SourceUnitId = tacticalUnitState.Id });
                tacticalBattlefield.SmokeEvents.Add(new TacticalSmokeEvent { Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId, Turn = turnState.TurnNumber, UnitId = tacticalUnitState.Id, Position = target, ActionPointCost = TacticalFireAndManeuver.SmokeActionPointCost, ExpiresAfterTurn = expires });
                CreateOrRefreshSmokeView(target);
                FinishTacticalSpecialOrder($"SMOKE DEPLOYED • ACTIVE THROUGH TURN {expires}");
                return true;
            }
            if (order == TacticalSpecialOrder.AreaSuppress)
            {
                TacticalLosResult los = TacticalLineOfSight.Inspect(localMovementBoard, tacticalUnitState.Position, target, TacticalFireAndManeuver.SuppressionRangeHexes);
                int chance = TacticalFireAndManeuver.AreaSuppressionChance(los);
                if (chance <= 0 || tacticalWeapon.RemainingAmmunition < TacticalFireAndManeuver.SuppressionAmmunitionCost ||
                    !tacticalUnitState.TrySpendActionPoints(TacticalFireAndManeuver.SuppressionActionPointCost))
                    return RejectTacticalSpecialOrder(target, chance <= 0 ? "NO SUPPRESSION LINE OF SIGHT" : "INSUFFICIENT AP OR AMMUNITION");
                int ammoBefore = tacticalWeapon.RemainingAmmunition;
                tacticalWeapon.RemainingAmmunition -= TacticalFireAndManeuver.SuppressionAmmunitionCost;
                tacticalUnitState.FiredThisTurn = true;
                if (TacticalConcealmentMorale.Reveal(tacticalUnitState))
                    RecordConcealmentEvent(TacticalConcealmentEventKind.Reveal, tacticalUnitState, null,
                        tacticalUnitState.Position, "Area suppression revealed firing position");
                int seed = TacticalDirectFire.CreateSeed(tacticalBattlefield.BattlefieldId, turnState.TurnNumber, tacticalEventSequence + 1);
                int roll = TacticalFireAndManeuver.DeterministicRoll(seed);
                TacticalUnitState defender = FindEnemyAt(target);
                int applied = defender != null && roll <= chance ? 2 : 0;
                if (defender != null)
                {
                    defender.WasFiredUponThisTurn = true;
                    if (applied > 0) defender.ApplySuppressionPoints(applied);
                }
                ResolvePendingFallback(defender, tacticalFriendlyStates);
                tacticalBattlefield.AreaSuppressionEvents.Add(new TacticalAreaSuppressionEvent { Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId, Turn = turnState.TurnNumber, Seed = seed, AttackerId = tacticalUnitState.Id, TargetId = defender?.Id, TargetPosition = target, Roll = roll, SuppressionChance = chance, SuppressionApplied = applied, AmmunitionBefore = ammoBefore, AmmunitionAfter = tacticalWeapon.RemainingAmmunition });
                FinishTacticalSpecialOrder(defender == null ? "AREA SUPPRESSED • NO CONFIRMED EFFECT" : applied > 0 ? "AREA SUPPRESSED • TARGET PINNED" : "AREA SUPPRESSED • NO EFFECT");
                return true;
            }
            if (order == TacticalSpecialOrder.Assault)
            {
                TacticalUnitState defender = FindEnemyAt(target);
                bool engineerAssault = InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Engineers;
                int chance = TacticalFireAndManeuver.AssaultChance(tacticalUnitState, defender, cell, engineerAssault);
                if (chance <= 0 || !tacticalUnitState.TrySpendActionPoints(TacticalFireAndManeuver.AssaultActionPointCost))
                    return RejectTacticalSpecialOrder(target, "ASSAULT REQUIRES ADJACENT ENEMY AND 4 AP");
                HexCoord origin = tacticalUnitState.Position;
                int seed = TacticalDirectFire.CreateSeed(tacticalBattlefield.BattlefieldId, turnState.TurnNumber, tacticalEventSequence + 1);
                int roll = TacticalFireAndManeuver.DeterministicRoll(seed);
                bool success = roll <= chance;
                if (TacticalConcealmentMorale.Reveal(tacticalUnitState))
                    RecordConcealmentEvent(TacticalConcealmentEventKind.Reveal, tacticalUnitState, defender,
                        tacticalUnitState.Position, "Close assault revealed concealed platoon");
                if (success)
                {
                ...
                ResolvePendingFallback(success ? defender : tacticalUnitState,
                    success ? tacticalFriendlyStates : tacticalEnemyStates);
            ...
            return RejectTacticalSpecialOrder(target, "ORDER NOT AVAILABLE");
        }

        private bool OrderPlannedFire(HexCoord target, TacticalFireMissionType type)
        {
            bool mortar = type == TacticalFireMissionType.MortarHighExplosive || type == TacticalFireMissionType.MortarSmoke;
            if (!TacticalPlannedFires.CanOrder(tacticalUnitState, tacticalBattlefield.FireSupport, target,
                    turnState.TurnNumber, mortar, out string rejection))
                return RejectTacticalSpecialOrder(target, rejection.ToUpperInvariant());
            TacticalVisibilityState contact = TacticalVisibilityState.Hidden;
            TacticalUnitState enemy = FindEnemyAt(target);
            if (enemy != null && tacticalContacts.TryGetValue(enemy.Id, out TacticalContactState report)) contact = report.State;
            else
                foreach (TacticalContactState known in tacticalContacts.Values)
                    if (known.LastKnownPosition.Equals(target) && known.State > contact) contact = known.State;
            bool qualityObserver = InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Reconnaissance;
            bool dangerClose = TacticalPlannedFires.IsDangerClose(tacticalFriendlyStates, target);
            TacticalPlannedFireMission mission = TacticalPlannedFires.Order(tacticalUnitState, tacticalBattlefield.FireSupport,
                type, target, turnState.TurnNumber, contact, qualityObserver, dangerClose, tacticalEventSequence + 1);
            if (mission == null) return RejectTacticalSpecialOrder(target, "FIRE MISSION COULD NOT BE ORDERED");
            tacticalBattlefield.PlannedFireMissions.Add(mission);
            tacticalBattlefield.FireMissionsRemaining = tacticalBattlefield.FireSupport.Ammunition;
            RecordPlannedFireEvent(mission, $"{FireMissionShortLabel(type)} requested on {target}; delivery turn {mission.DeliveryTurn}");
            RefreshPlannedFireViews();
            string warning = dangerClose ? " • DANGER CLOSE" : string.Empty;
            FinishTacticalSpecialOrder($"{FireMissionShortLabel(type)} REQUESTED • T-{mission.DeliveryTurn - turnState.TurnNumber} • SCATTER {mission.FinalScatterRadius}{warning}");
            return true;
        }

        private TacticalFireMissionType SelectedMortarMissionType()
            => tacticalFireMissionType == TacticalFireMissionType.Smoke
                ? TacticalFireMissionType.MortarSmoke : TacticalFireMissionType.MortarHighExplosive;

        private void CycleFireMissionType()
        {
            tacticalFireMissionType = tacticalFireMissionType == TacticalFireMissionType.HighExplosive
                ? TacticalFireMissionType.Smoke
                : tacticalFireMissionType == TacticalFireMissionType.Smoke
                    ? TacticalFireMissionType.Illumination : TacticalFireMissionType.HighExplosive;
            tacticalOrderFeedback = $"FIRE-SUPPORT AMMUNITION • {FireMissionShortLabel(tacticalFireMissionType)}";
        }

        private void IssueAdjustFire()
        {
            TacticalPlannedFireMission reference = tacticalBattlefield.PlannedFireMissions.FindLast(item => item.Status == TacticalFireMissionStatus.Delivered);
            if (reference == null) reference = tacticalBattlefield.PlannedFireMissions.FindLast(item => item.Status == TacticalFireMissionStatus.Pending);
            if (reference == null || !TacticalPlannedFires.TryAdjust(tacticalUnitState, tacticalBattlefield.FireSupport, reference.RequestedHex))
            {
                tacticalOrderFeedback = "ADJUST FIRE REQUIRES A PREVIOUS AIM POINT AND 1 AP";
                tacticalAudio.Play(TacticalSound.OrderCancel);
                return;
            }
            RefreshPlannedFireViews();
            tacticalMenuOpen = false;
            tacticalOrderFeedback = $"FIRE ADJUSTED • {reference.RequestedHex} • LEVEL {tacticalBattlefield.FireSupport.AdjustmentLevel}";
            tacticalAudio.Play(TacticalSound.OrderConfirm);
        }

        private void IssueMortarSetup()
        {
            bool weapons = InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Weapons;
            if (!TacticalPlannedFires.TrySetupMortar(tacticalUnitState, weapons))
            {
                tacticalOrderFeedback = "MORTAR SETUP REQUIRES WEAPONS PLATOON, 2 AP, AND NO MOVE OR FIRE";
                tacticalAudio.Play(TacticalSound.OrderCancel);
                return;
            }
            tacticalMenuOpen = false;
            tacticalOrderFeedback = $"MORTARS EMPLACED • {tacticalUnitState.MortarRounds} ROUNDS";
            tacticalAudio.Play(TacticalSound.OrderConfirm);
        }

        private bool RejectTacticalSpecialOrder(HexCoord target, string reason)
        {
            tacticalOrderFeedback = reason;
            if (localCells.TryGetValue(target, out HexCellView cell)) cell.SetInvalid(true);
            tacticalAudio.Play(TacticalSound.OrderCancel);
            return false;
        }
        ...
        {
            if (tacticalSpecialPlanning == TacticalSpecialOrder.AreaSuppress || tacticalSpecialPlanning == TacticalSpecialOrder.Smoke ||
                tacticalSpecialPlanning == TacticalSpecialOrder.Assault || tacticalSpecialPlanning == TacticalSpecialOrder.CallForFire)
                AdvanceTutorial(4);
        ...
        private void CreateOrRefreshSmokeView(HexCoord position)
        {
            if (tacticalSmokeViews.TryGetValue(position, out GameObject existing)) Destroy(existing);
            GameObject smoke = GameObject.CreatePrimitive(PrimitiveType.Sphere);
            smoke.name = "Smoke " + position;
            smoke.transform.SetParent(tacticalRoot.transform, false);
            smoke.transform.position = localCells[position].transform.position + Vector3.up * .48f;
            smoke.transform.localScale = new Vector3(1.35f, .62f, 1.35f);
            smoke.GetComponent<MeshRenderer>().sharedMaterial = NewOverlayMaterial(new Color(.72f, .75f, .72f, .48f));
            Destroy(smoke.GetComponent<Collider>());
            tacticalSmokeViews[position] = smoke;
        }

        private void DecayTacticalSmoke()
        {
            tacticalBattlefield.ActiveSmokeMarkers.RemoveAll(marker => !marker.IsActive(turnState.TurnNumber));
            var expired = new List<HexCoord>();
            foreach (KeyValuePair<HexCoord, GameObject> pair in tacticalSmokeViews)
            {
                if (localMovementBoard[pair.Key].HasActiveSmoke(turnState.TurnNumber)) continue;
                localMovementBoard[pair.Key].SmokeExpiresAfterTurn = 0;
                Destroy(pair.Value);
                expired.Add(pair.Key);
            }
            foreach (HexCoord coord in expired) tacticalSmokeViews.Remove(coord);
        }
        ...
        private void PreviewTacticalRoute(HexCellView destination)
        {
            ...
            tacticalPreviewCost = TacticalFireAndManeuver.MovementCost(route.ActionPointCost, tacticalUnitState.MovementPosture);
        ...
        private bool TryIssueTacticalMove(HexCoord destination)
        {
            ...
            TacticalRouteResult route = TacticalMovementPlanner.FindRoute(
                localMovementBoard, origin, destination,
                TacticalFireAndManeuver.PlanningBudget(before, tacticalUnitState.MovementPosture), tacticalUnitState.Id);
            int movementCost = route.IsValid ? TacticalFireAndManeuver.MovementCost(route.ActionPointCost, tacticalUnitState.MovementPosture) : 0;
            if (!route.IsValid || !tacticalUnitState.TryBeginMove(movementCost))
            {
        ...
        private IEnumerator MoveTacticalUnit(HexCoord origin, HexCoord destination, int actionPointsBefore, TacticalRouteResult route)
        {
            ...
            if (TacticalConcealmentMorale.Reveal(tacticalUnitState))
                RecordConcealmentEvent(TacticalConcealmentEventKind.Reveal, tacticalUnitState, null, origin,
                    "Movement revealed concealed platoon", 0, origin);
            bool reactionTriggered = false;
            bool obstacleHalted = false;
            string obstacleSummary = null;
            ...
            for (int index = 1; index < route.Path.Count; index++)
            {
                ...
                if (ResolveTacticalObstacleEntry(tacticalUnitState, stepCoord, "USMC", out obstacleSummary))
                {
                    obstacleHalted = true;
                    reachedIndex = index;
                }
                ...
                if (obstacleHalted) break;
            }
            HexCoord finalPosition = route.Path[reachedIndex];
            bool completed = reachedIndex == route.Path.Count - 1 && !obstacleHalted;
            localMovementBoard[origin].OccupantId = null;
            ...
            RecordTacticalMovement(origin, finalPosition, TacticalFireAndManeuver.MovementCost(route.ActionPointCost, tacticalUnitState.MovementPosture), actionPointsBefore,
                ...
                completed ? "Terrain, slope, and known-obstacle cost applied" : obstacleHalted ? obstacleSummary : "Halted by reaction fire", route.Path.GetRange(0, reachedIndex + 1));
            RefreshTacticalObservation();
            tacticalOrderFeedback = completed
                ? $"MOVE COMPLETE • {tacticalUnitState.RemainingActionPoints} AP REMAIN"
                : obstacleHalted
                    ? $"MOVE HALTED • {obstacleSummary.ToUpperInvariant()} • {tacticalUnitState.RemainingActionPoints} AP REMAIN"
        ...
        private IEnumerator ResolveTacticalReaction(TacticalUnitState reactor, TacticalWeaponState reactorWeapon, HexCoord triggerPosition, TacticalLosResult los)
        {
            ...
            ApplyBattleConditionsModifier(preview);
            TacticalConcealmentMorale.ApplyAmbushModifier(preview, reactor);
            bool wasAmbush = reactor.IsConcealed && reactor.AmbushReady;
            ...
            if (TacticalConcealmentMorale.Reveal(reactor))
                RecordConcealmentEvent(wasAmbush ? TacticalConcealmentEventKind.Ambush : TacticalConcealmentEventKind.Reveal,
                    reactor, tacticalUnitState, reactor.Position, wasAmbush ? "Ambush reaction fired; position revealed" : "Reaction fire revealed position");
            tacticalUnitState.WasFiredUponThisTurn = true;
            fireEvent.Sequence = ++tacticalEventSequence;
            fireEvent.BattlefieldId = tacticalBattlefield.BattlefieldId;
            fireEvent.Turn = turnState.TurnNumber;
        ...
        private void CancelTacticalInteraction(bool recordCancellation)
        {
            ...
            if (recordCancellation && (tacticalMovePlanning || tacticalLosPlanning || tacticalFirePlanning || tacticalReconPlanning || tacticalSpecialPlanning != TacticalSpecialOrder.None || tacticalMenuOpen))
                tacticalAudio.Play(TacticalSound.OrderCancel);
        ...
        private void UpdatePointer()
        {
            ...
            Vector2 guiPointer = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y) / GetUiScale();
            if (counterMenuOpen && counterMenuRect.Contains(guiPointer) ||
                !hudCollapsed && (endTurnRect.Contains(guiPointer) || enterTacticalRect.Contains(guiPointer))) return;
        ...
        private void EndTurn()
        {
            ...
            foreach (TacticalUnitState tacticalFriendly in tacticalFriendlyStates)
            {
                tacticalFriendly.BeginTurn();
                if (tacticalFriendlyViews.TryGetValue(tacticalFriendly.Id, out TacticalFriendlyView friendlyView)) PresentFriendly(friendlyView);
            }
            unit.Present(unitState);
            SyncOperationalFromUnit(unitState);
            SaveOperationalScenario();
        ...
        {
            if (tacticalUnitMoving || tacticalFireResolving || tacticalEnemyTurnActive || resultScreenActive || campaignBriefingActive || settingsPanelOpen || supportCardModalActive || tacticalForcePanelOpen || tacticalDeploymentActive) return;
            CancelTacticalInteraction(false);
            AdvanceTutorial(5);
            tacticalAudio.Play(TacticalSound.EndTurn);
        ...
        private IEnumerator RunEnemyTurn()
        {
            ...
            foreach (TacticalUnitState enemy in tacticalEnemyStates)
            {
                ...
                tacticalEnemyContacts.TryGetValue(enemy.Id, out TacticalContactState previousEnemyContact);
                TacticalUnitState opponent = FindBestTargetForEnemy(enemy, previousEnemyContact, out TacticalContactState opponentContact);
                TacticalAiOrder order = PlanEnemyFireEvasion(enemy, seed) ??
                    TacticalEnemyTurn.PlanOrder(localMovementBoard, enemy, weapon,
                        tacticalObjective.ObjectiveHex, opponent, opponentContact, turnState.TurnNumber, seed);
                if (!TacticalEnemyTurn.ValidateOrder(localMovementBoard, order, enemy, weapon,
                        opponent, opponentContact, turnState.TurnNumber, out string rejection))
                {
                ...
                yield return ExecuteEnemyOrder(enemy, weapon, order, opponent, opponentContact, visible);
            ...
            ResolveDueFireMissions();
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
            {
                friendly.BeginTurn();
                PresentFriendly(tacticalFriendlyViews[friendly.Id]);
            }
        ...
        private TacticalAiOrder PlanEnemyFireEvasion(TacticalUnitState enemy, int seed)
        {
            if (enemy == null || !enemy.CanMove || tacticalBattlefield?.PlannedFireMissions == null) return null;
            var threats = tacticalBattlefield.PlannedFireMissions.FindAll(item =>
                item.Status == TacticalFireMissionStatus.Pending && item.DeliveryTurn <= turnState.TurnNumber + 1 &&
                HexCoord.Distance(enemy.Position, item.RequestedHex) <= TacticalPlannedFires.EffectRadiusHexes);
            if (threats.Count == 0) return null;
            Dictionary<HexCoord, int> reachable = TacticalMovementPlanner.Reachable(
                localMovementBoard, enemy.Position, enemy.RemainingActionPoints, enemy.Id, "PLA");
            HexCoord best = enemy.Position;
            int bestDistance = 0;
            foreach (TacticalPlannedFireMission threat in threats) bestDistance += HexCoord.Distance(best, threat.RequestedHex);
            foreach (HexCoord candidate in reachable.Keys)
            {
                int distance = 0;
                foreach (TacticalPlannedFireMission threat in threats) distance += HexCoord.Distance(candidate, threat.RequestedHex);
                if (distance <= bestDistance) continue;
                best = candidate;
                bestDistance = distance;
            }
            if (best.Equals(enemy.Position)) return null;
            TacticalRouteResult route = TacticalMovementPlanner.FindRoute(localMovementBoard, enemy.Position, best,
                enemy.RemainingActionPoints, enemy.Id, "PLA");
            if (!route.IsValid) return null;
            return new TacticalAiOrder
            {
                UnitId = enemy.Id, Kind = TacticalAiOrderKind.Move, Origin = enemy.Position, Destination = best,
                ActionPointCost = route.ActionPointCost, Path = new List<HexCoord>(route.Path), Seed = seed,
                Intent = "Evade suspected incoming fire"
            };
        }
        ...
        private void EvaluateTacticalVictory()
        {
            ...
            TacticalBattleOutcome outcome = TacticalVictory.Evaluate(tacticalObjective, tacticalFriendlyStates, tacticalEnemyStates,
            ...
            CompleteStandaloneProgression(outcome);
            ...
            {
                ApplyOperationalBattleLosses();
                SaveOperationalScenario();
                Debug.Log($"ALWAYS_FAITHFUL_OPERATIONAL_LOSSES_APPLIED friendly={activeOperationalBattalion.Id}/{activeOperationalBattalion.Strength}% enemy={tacticalOperationalEnemyBattalion?.Id}/{tacticalOperationalEnemyBattalion?.Strength}%");
            }
            else if (battalionStatus != null)
            {
                int strengthBefore = battalionStatus.Strength;
                TacticalBattalion.ApplyEngagementResult(battalionStatus, outcome, WorstFriendlyStatus());
        ...
        private void ApplyOperationalBattleLosses()
        {
            ApplyPlatoonLosses(activeOperationalBattalion, tacticalFriendlyStates);
            ApplyPlatoonLosses(tacticalOperationalEnemyBattalion, tacticalEnemyStates);
        }

        private static void ApplyPlatoonLosses(OperationalBattalionState battalion, List<TacticalUnitState> tacticalUnits)
        {
            if (battalion?.Platoons == null || tacticalUnits == null) return;
            foreach (OperationalPlatoonState platoon in battalion.Platoons)
            {
                TacticalUnitState resolved = tacticalUnits.Find(unit => unit.Id == platoon.Id);
                if (resolved == null) continue;
                platoon.MaximumStrength = Math.Max(1, resolved.MaximumStrength);
                platoon.Strength = Math.Max(0, Math.Min(platoon.MaximumStrength, resolved.Strength));
            }
            OperationalScenarioRules.SynchronizeBattalionStrength(battalion);
        }

        private void CompleteStandaloneProgression(TacticalBattleOutcome outcome)
        {
            if (activeBattleRequest != null) return;
            int initialAmmo = 0;
            int remainingAmmo = 0;
            foreach (TacticalWeaponState weapon in tacticalFriendlyWeapons.Values)
            {
                initialAmmo += weapon.MaximumAmmunition;
                remainingAmmo += weapon.RemainingAmmunition;
            }
            foreach (TacticalSecondaryObjectiveState secondary in tacticalBattlefield.SecondaryObjectives)
            {
                if (secondary.Type == TacticalSecondaryObjectiveType.PreserveForce)
                    secondary.Completed = tacticalFriendlyStates.TrueForAll(unit => unit.Strength >= 50);
                else if (secondary.Type == TacticalSecondaryObjectiveType.AmmunitionDiscipline)
                    secondary.Completed = remainingAmmo * 2 >= initialAmmo;
                else if (secondary.Type == TacticalSecondaryObjectiveType.BreachRoute)
                    secondary.Completed = tacticalBattlefield.Obstacles != null && tacticalBattlefield.Obstacles.Exists(item => item.IsBreached);
                else
                    secondary.Completed = tacticalObjective.ObservedEnemyIds.Count >= tacticalEnemyStates.Count;
                secondary.Failed = !secondary.Completed;
            }
            var report = new TacticalAfterActionReport
            {
                ScenarioName = tacticalBattlefield.QuickBattle?.ScenarioName ?? "Standalone Battle",
                Outcome = outcome,
                Turns = tacticalObjective.OutcomeTurn - tacticalObjective.BattleStartTurn + 1,
                AmmunitionExpended = initialAmmo - remainingAmmo,
                FireMissionsExpended = tacticalBattlefield.FireSupport.MaximumAmmunition - tacticalBattlefield.FireSupport.Ammunition
            };
            foreach (TacticalUnitState friendly in tacticalFriendlyStates) report.FriendlyStrengthRemaining += friendly.Strength;
            foreach (TacticalUnitState enemy in tacticalEnemyStates) report.EnemyStrengthRemaining += enemy.Strength;
            report.SecondaryObjectivesCompleted = tacticalBattlefield.SecondaryObjectives.FindAll(item => item.Completed).Count;
            report.Highlights.Add($"{tacticalBattlefield.FireEvents.Count} direct-fire engagements; {tacticalBattlefield.ReactionEvents.Count} reactions.");
            int deliveredFires = tacticalBattlefield.PlannedFireMissions?.FindAll(item => item.Status == TacticalFireMissionStatus.Delivered).Count ?? 0;
            report.Highlights.Add($"{tacticalBattlefield.AssaultEvents.Count} assaults; {deliveredFires} planned fire missions delivered.");
            List<TacticalConcealmentEvent> concealmentEvents = tacticalBattlefield.ConcealmentEvents ?? new List<TacticalConcealmentEvent>();
            int ambushes = concealmentEvents.FindAll(item => item.Kind == TacticalConcealmentEventKind.Ambush).Count;
            int fallbacks = concealmentEvents.FindAll(item => item.Kind == TacticalConcealmentEventKind.FallBack || item.Kind == TacticalConcealmentEventKind.Surrender).Count;
            report.Highlights.Add($"{ambushes} ambushes; {fallbacks} forced fallbacks or surrenders.");
            int obstacleContacts = tacticalBattlefield.ObstacleEvents?.FindAll(item => item.Kind == TacticalObstacleEventKind.MovementHalted).Count ?? 0;
            int breachLanes = tacticalBattlefield.Obstacles?.FindAll(item => item.IsBreached).Count ?? 0;
            report.Highlights.Add($"{obstacleContacts} obstacle contacts halted movement; {breachLanes} safe lanes opened.");
            tacticalBattlefield.AfterActionReport = report;

            tacticalCareer.Battles++;
            bool victory = outcome == TacticalBattleOutcome.UsmcVictory;
            if (victory) tacticalCareer.Victories++;
            int secondaryExperience = 0;
            foreach (TacticalSecondaryObjectiveState item in tacticalBattlefield.SecondaryObjectives)
                if (item.Completed) secondaryExperience += item.RewardPoints;
            foreach (TacticalUnitState unit in tacticalFriendlyStates)
            {
                TacticalPlatoonCareerRecord record = tacticalCareer.Platoons.Find(item => item.UnitId == unit.Id);
                if (record == null)
                {
                    record = new TacticalPlatoonCareerRecord { UnitId = unit.Id, DisplayName = unit.DisplayName };
                    tacticalCareer.Platoons.Add(record);
                }
                record.DisplayName = unit.DisplayName;
                record.Battles++;
                if (victory) record.Victories++;
                record.Experience += 1 + (victory ? 1 : 0) + secondaryExperience;
                record.Strength = unit.Strength;
            }
            SaveTacticalCareer();
        }
        ...
        private void WriteBattleResult()
        {
            ...
            var result = new BattleResult
            {
                ...
                UsmcCasualties = CountReduced(tacticalFriendlyStates),
            ...
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
            {
                ...
                {
                    Role = FriendlyContractRole(InferFriendlyRole(friendly.Id)),
                    UnitId = friendly.Id,
                    DisplayName = friendly.DisplayName,
                    FinalStatus = friendly.CombatStatus
                });
            }
        ...
        private static string FriendlyContractRole(TacticalPlatoonRole role)
        {
            switch (role)
            {
                case TacticalPlatoonRole.Weapons: return "usmc-weapons-platoon";
                case TacticalPlatoonRole.Reconnaissance: return "usmc-recon-platoon";
                case TacticalPlatoonRole.Engineers: return "usmc-engineer-platoon";
                default: return "usmc-rifle-platoon";
            }
        }
        ...
        {
            TacticalUnitState friendly = FindFriendly(id);
            if (friendly != null) return friendly.DisplayName;
        ...
        private List<string> BuildTacticalEventLog(int maximumEntries)
        {
            ...
            if (tacticalBattlefield.SmokeEvents != null)
                foreach (TacticalSmokeEvent smoke in tacticalBattlefield.SmokeEvents)
                    entries.Add((smoke.Sequence, $"SMOKE • T{smoke.Turn} • {UnitDisplayName(smoke.UnitId)} screened {smoke.Position}"));
            if (tacticalBattlefield.AreaSuppressionEvents != null)
                foreach (TacticalAreaSuppressionEvent suppression in tacticalBattlefield.AreaSuppressionEvents)
                    entries.Add((suppression.Sequence, $"AREA FIRE • T{suppression.Turn} • {UnitDisplayName(suppression.AttackerId)} suppressed {suppression.TargetPosition} • {suppression.SuppressionApplied} pts"));
            ...
            if (tacticalBattlefield.PositionEvents != null)
                foreach (TacticalPositionEvent position in tacticalBattlefield.PositionEvents)
                    entries.Add((position.Sequence, $"POSITION • T{position.Turn} • {UnitDisplayName(position.UnitId)} L{position.LevelBefore}→L{position.LevelAfter} at {position.Position} • face {position.FacingSector + 1}"));
            ...
            if (tacticalBattlefield.PlannedFireEvents != null)
                foreach (TacticalPlannedFireEvent fireMission in tacticalBattlefield.PlannedFireEvents)
                    entries.Add((fireMission.Sequence, $"PLANNED FIRE • T{fireMission.Turn} • {FireMissionShortLabel(fireMission.Type)} • {fireMission.Summary}"));
            if (tacticalBattlefield.ConcealmentEvents != null)
                foreach (TacticalConcealmentEvent concealment in tacticalBattlefield.ConcealmentEvents)
                    entries.Add((concealment.Sequence, $"{concealment.Kind.ToString().ToUpperInvariant()} - T{concealment.Turn} - {UnitDisplayName(concealment.UnitId)} - {concealment.Summary}"));
            if (tacticalBattlefield.ObstacleEvents != null)
                foreach (TacticalObstacleEvent obstacle in tacticalBattlefield.ObstacleEvents)
                    entries.Add((obstacle.Sequence, $"OBSTACLE - T{obstacle.Turn} - {UnitDisplayName(obstacle.UnitId)} - {obstacle.Position} - {obstacle.Summary}"));
        ...
        private IEnumerator ExecuteEnemyOrder(TacticalUnitState enemy, TacticalWeaponState weapon,
            TacticalAiOrder order, TacticalUnitState opponent, TacticalContactState opponentContact, bool visible)
        {
            switch (order.Kind)
            {
                ...
                case TacticalAiOrderKind.Fire:
                    yield return ExecuteEnemyFire(enemy, weapon, order, opponent, opponentContact, visible);
                    break;
                case TacticalAiOrderKind.Smoke:
                    if (enemy.TrySpendActionPoints(TacticalFireAndManeuver.SmokeActionPointCost) &&
                        localMovementBoard.TryGetValue(enemy.Position, out TacticalMovementCell smokeCell))
                    {
                        int expires = turnState.TurnNumber + TacticalFireAndManeuver.SmokeDurationTurns - 1;
                        smokeCell.SmokeExpiresAfterTurn = expires;
                        tacticalBattlefield.ActiveSmokeMarkers.RemoveAll(item => item.Position.Equals(enemy.Position));
                        tacticalBattlefield.ActiveSmokeMarkers.Add(new TacticalSmokeMarker
                        {
                            Position = enemy.Position, PlacedTurn = turnState.TurnNumber,
                            ExpiresAfterTurn = expires, SourceUnitId = enemy.Id
                        });
                        tacticalBattlefield.SmokeEvents.Add(new TacticalSmokeEvent
                        {
                            Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId,
                            Turn = turnState.TurnNumber, UnitId = enemy.Id, Position = enemy.Position,
                            ActionPointCost = TacticalFireAndManeuver.SmokeActionPointCost, ExpiresAfterTurn = expires
                        });
                        CreateOrRefreshSmokeView(enemy.Position);
                    }
                    break;
                case TacticalAiOrderKind.AreaSuppress:
                    if (enemy.TrySpendActionPoints(TacticalFireAndManeuver.SuppressionActionPointCost) &&
                        weapon.RemainingAmmunition >= TacticalFireAndManeuver.SuppressionAmmunitionCost)
                    {
                        int ammoBefore = weapon.RemainingAmmunition;
                        weapon.RemainingAmmunition -= TacticalFireAndManeuver.SuppressionAmmunitionCost;
                        enemy.FiredThisTurn = true;
                        int applied = opponent != null && opponent.Position.Equals(order.Destination) ? 20 : 0;
                        if (applied > 0) opponent.ApplySuppressionPoints(applied);
                        tacticalBattlefield.AreaSuppressionEvents.Add(new TacticalAreaSuppressionEvent
                        {
                            Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId,
                            Turn = turnState.TurnNumber, Seed = order.Seed, AttackerId = enemy.Id,
                            TargetId = applied > 0 ? opponent.Id : null, TargetPosition = order.Destination,
                            Roll = 1, SuppressionChance = 100, SuppressionApplied = applied,
                            AmmunitionBefore = ammoBefore, AmmunitionAfter = weapon.RemainingAmmunition
                        });
                    }
                    break;
                ...
                case TacticalAiOrderKind.Hide:
                    if (localMovementBoard.TryGetValue(enemy.Position, out TacticalMovementCell hideCell) &&
                        TacticalConcealmentMorale.TryHide(enemy, hideCell, turnState.TurnNumber))
                        RecordConcealmentEvent(TacticalConcealmentEventKind.Hide, enemy, null, enemy.Position,
                            "Enemy concealed in covered terrain; ambush prepared", TacticalConcealmentMorale.HideActionPointCost);
                    break;
                case TacticalAiOrderKind.Search:
                    if (TacticalConcealmentMorale.TrySearch(enemy, order.Destination))
                    {
                        bool revealed = opponent != null && opponent.Position.Equals(order.Destination) &&
                            TacticalConcealmentMorale.Reveal(opponent);
                        RecordConcealmentEvent(TacticalConcealmentEventKind.Search, enemy, revealed ? opponent : null,
                            order.Destination, revealed ? "Enemy search exposed concealed USMC platoon" : "Enemy searched suspected position",
                            TacticalConcealmentMorale.SearchActionPointCost);
                    }
                    break;
                case TacticalAiOrderKind.Withdraw:
                    HexCoord origin = enemy.Position;
                    if (TacticalConcealmentMorale.TryWithdraw(enemy, order.Destination, localMovementBoard, tacticalFriendlyStates))
                    {
                        localMovementBoard[origin].OccupantId = null;
                        localMovementBoard[order.Destination].OccupantId = enemy.Id;
                        enemy.Position = order.Destination;
                        if (tacticalContactViews.TryGetValue(enemy.Id, out ContactMarkerView marker))
                            marker.transform.position = LocalCounterPosition(order.Destination) + Vector3.up * .03f;
                        RecordConcealmentEvent(TacticalConcealmentEventKind.Withdraw, enemy, null, order.Destination,
                            $"Enemy withdrew from {origin} to {order.Destination}", TacticalConcealmentMorale.WithdrawActionPointCost, origin);
                    }
                    break;
        ...
        private IEnumerator ExecuteEnemyMove(TacticalUnitState enemy, TacticalAiOrder order, bool visible)
        {
            ...
            HexCoord origin = enemy.Position;
            if (TacticalConcealmentMorale.Reveal(enemy))
                RecordConcealmentEvent(TacticalConcealmentEventKind.Reveal, enemy, null, origin,
                    "Movement revealed concealed platoon", 0, origin);
            ContactMarkerView marker = tacticalContactViews[enemy.Id];
            bool obstacleHalted = false;
            string obstacleSummary = null;
            int reachedIndex = order.Path.Count - 1;
            for (int index = 1; index < order.Path.Count; index++)
            {
                HexCoord step = order.Path[index];
                if (visible)
                {
                ...
                if (ResolveTacticalObstacleEntry(enemy, step, "PLA", out obstacleSummary))
                {
                    obstacleHalted = true;
                    reachedIndex = index;
                }
                if (visible && ResolveFriendlyReaction(enemy, step))
                {
                    reachedIndex = index;
                    break;
                }
            ...
            HexCoord finalPosition = order.Path[reachedIndex];
            localMovementBoard[origin].OccupantId = null;
            ...
            ResolvePendingFallback(enemy, tacticalFriendlyStates);
        }

        private bool ResolveFriendlyReaction(TacticalUnitState mover, HexCoord triggerPosition)
        {
            var candidates = new List<TacticalReactionCandidate>();
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
                if (tacticalFriendlyWeapons.TryGetValue(friendly.Id, out TacticalWeaponState weapon))
                    candidates.Add(new TacticalReactionCandidate(friendly, weapon));
            TacticalReactionCandidate? selected = TacticalReactionFire.SelectReactor(localMovementBoard, candidates, triggerPosition, out TacticalLosResult los);
            if (!selected.HasValue) return false;
            TacticalUnitState reactor = selected.Value.Unit;
            ...
            if (TacticalConcealmentMorale.Reveal(reactor))
                RecordConcealmentEvent(wasAmbush ? TacticalConcealmentEventKind.Ambush : TacticalConcealmentEventKind.Reveal,
                    reactor, mover, reactor.Position, wasAmbush ? "Ambush reaction fired; position revealed" : "Reaction fire revealed position");
            ...
            {
                Sequence = ++tacticalEventSequence, BattlefieldId = tacticalBattlefield.BattlefieldId, Turn = turnState.TurnNumber,
                Seed = seed, ReactorId = reactor.Id, MoverId = mover.Id, ReactorPosition = reactor.Position,
                TriggerPosition = triggerPosition, RangeHexes = fire.RangeHexes, HitChance = fire.HitChance,
                SuppressionChance = fire.SuppressionChance, Roll = fire.Roll, Outcome = fire.Outcome,
                Resolution = TacticalReactionFire.ResolutionFor(fire.Outcome), AmmunitionBefore = fire.AmmunitionBefore,
                AmmunitionAfter = fire.AmmunitionAfter, Modifiers = fire.Modifiers
            });
            if (suppression != null)
            {
            ...
            PresentFriendly(tacticalFriendlyViews[reactor.Id]);
            tacticalAudio.Play(fire.Outcome == TacticalFireOutcome.Miss ? TacticalSound.FireMiss : TacticalSound.Interrupt);
            return fire.Outcome != TacticalFireOutcome.Miss;
        }
            ...
            TacticalAiOrder order, TacticalUnitState opponent, TacticalContactState opponentContact, bool visible)
        {
            ...
            ApplyBattleConditionsModifier(preview);
            TacticalConcealmentMorale.ApplyAmbushModifier(preview, enemy);
            bool wasAmbush = enemy.IsConcealed && enemy.AmbushReady;
            ...
            if (TacticalConcealmentMorale.Reveal(enemy))
                RecordConcealmentEvent(wasAmbush ? TacticalConcealmentEventKind.Ambush : TacticalConcealmentEventKind.Reveal,
                    enemy, opponent, enemy.Position, wasAmbush ? "Prepared ambush fired; position revealed" : "Firing revealed position");
            ...
            ResolvePendingFallback(opponent, tacticalEnemyStates);
            if (suppression != null)
            {
                suppression.Sequence = ++tacticalEventSequence;
                suppression.BattlefieldId = tacticalBattlefield.BattlefieldId;
            ...
            TacticalFriendlyView targetView = tacticalFriendlyViews[opponent.Id];
            PresentFriendly(targetView);
            targetView.Counter.CueIncomingFire(fire.Outcome);
            targetView.Symbol.FlashIcon(IncomingFireFlashColor(fire.Outcome));
            targetView.Miniature.FlashTint(IncomingFireFlashColor(fire.Outcome));
            tacticalFireLine.enabled = true;
            ...
            tacticalFireLine.SetPosition(1, localCells[opponent.Position].transform.position + Vector3.up * (CellSurfaceOffset + .38f));
        ...
        private IEnumerator RunHudRegression()
        {
            ...
            // This regression exercises the in-battle HUD after the normal
            // force/deployment workflow has been completed by the player.
            tacticalForcePanelOpen = false;
            tacticalDeploymentActive = false;
            campaignBriefingActive = false;
            ...
            OpenTacticalMenu(new Vector2(640f, 360f));
            ToggleHudVisibility();
            bool collapsedCleanly = hudCollapsed && tacticalMenuOpen && !tacticalMovePlanning &&
                !tacticalLosPlanning && !tacticalFirePlanning && !tacticalReconPlanning;
            CancelTacticalInteraction(false);
            OpenTacticalMenu(new Vector2(640f, 360f));
            bool contextMenuIndependent = hudCollapsed && tacticalMenuOpen;
            ToggleHudVisibility();
            CancelTacticalInteraction(false);
            if (!collapsedCleanly || !contextMenuIndependent || hudCollapsed)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_HUD_REGRESSION_FAILED collapse={collapsedCleanly} contextIndependent={contextMenuIndependent} restored={!hudCollapsed}");
                Application.Quit(1);
                yield break;
            }
            ...
            bool reactionReady = TacticalFireAndManeuver.CanReact(tacticalUnitState) && tacticalWeapon.RemainingAmmunition > 0;
            if (!reactionReady)
            {
            ...
            Debug.Log($"ALWAYS_FAITHFUL_HUD_REGRESSION_OK overlayCells={overlayCellsAfterToggleOn} logEntries={logAfter.Count} reactionReady={reactionReady} collapse={collapsedCleanly}");
            Application.Quit(0);
        }
        ...
        private IEnumerator RunCombatPowerRegression()
        {
            yield return null;
            TacticalAttackProfile hasty = TacticalCombatPower.AttackProfile(TacticalAttackType.Hasty);
            TacticalAttackProfile deliberate = TacticalCombatPower.AttackProfile(TacticalAttackType.Deliberate);
            TacticalAttackProfile rapid = TacticalCombatPower.AttackProfile(TacticalAttackType.Rapid);
            var unit = new TacticalUnitState("power-probe", "Power Probe", new HexCoord(1, 1), 12);
            TacticalStrengthEvent loss = TacticalCombatPower.ApplyStrengthDamage(unit, deliberate.StrengthDamage, "test");
            bool separated = unit.Strength == 75 && unit.SuppressionPoints == 0 && loss.StrengthBefore == 100;
            bool entrenched = TacticalCombatPower.TryEntrench(unit) && unit.EntrenchmentLevel == 1 &&
                TacticalCombatPower.EntrenchmentHitModifier(unit, unit.Position) == TacticalCombatPower.EntrenchmentPenaltyPerLevel;
            int frontPosition = TacticalCombatPower.EntrenchmentHitModifier(unit, unit.Position, new HexCoord(1, 0));
            int rearPosition = TacticalCombatPower.EntrenchmentHitModifier(unit, unit.Position, new HexCoord(1, 2));
            bool directionalPosition = frontPosition == TacticalCombatPower.EntrenchmentPenaltyPerLevel && rearPosition == 0;
            TacticalFireAndManeuver.RecordMove(unit, TacticalMovementPosture.Tactical,
                new List<HexCoord> { new HexCoord(1, 1), new HexCoord(1, 2) });
            bool positionLost = unit.EntrenchmentLevel == 0;
            unit.BeginTurn();
            unit.Strength = 60;
            unit.Supply = 4;
            unit.ApplySuppressionPoints(40);
            bool reorganized = TacticalCombatPower.TryReorganize(unit) && unit.Strength == 75 && unit.Supply == 2 && unit.SuppressionPoints == 0;
            if (hasty.ActionPointCost >= deliberate.ActionPointCost || rapid.AmmunitionCost != 2 || !separated || !entrenched ||
                !directionalPosition || !positionLost || !reorganized)
            {
                Debug.LogError("ALWAYS_FAITHFUL_COMBAT_POWER_REGRESSION_FAILED profiles/strength/position/reorganization");
                Application.Quit(1);
                yield break;
            }
            Debug.Log("ALWAYS_FAITHFUL_COMBAT_POWER_REGRESSION_OK");
            Application.Quit(0);
        }

        private IEnumerator RunLongevityRegression()
        {
            yield return null;
            var config = new TacticalQuickBattleConfiguration
            {
                ScenarioName = "Regression Scenario", Seed = 4242, MissionType = TacticalMissionType.ReconInForce,
                Difficulty = TacticalDifficulty.Elite, Weather = TacticalWeather.Storm,
                Visibility = TacticalVisibility.Night, TurnLimit = 12, TutorialEnabled = true
            };
            int fireModifier = TacticalStandaloneRules.FireModifier(config);
            int observationRange = TacticalStandaloneRules.ObservationRange(config, TacticalObservation.ClearObservationRangeHexes);
            List<TacticalSecondaryObjectiveState> objectives = TacticalStandaloneRules.CreateSecondaryObjectives(config.MissionType);
            var battlefield = new TacticalBattlefieldState { QuickBattle = config, SecondaryObjectives = objectives,
                Tutorial = new TacticalTutorialState { Enabled = true } };
            string json = JsonUtility.ToJson(battlefield);
            TacticalBattlefieldState restored = JsonUtility.FromJson<TacticalBattlefieldState>(json);
            if (fireModifier >= 0 || observationRange >= TacticalObservation.ClearObservationRangeHexes || objectives.Count != 3 ||
                TacticalStandaloneRules.EnemyStrength(config.Difficulty) != 115 || string.IsNullOrEmpty(TacticalStandaloneRules.TutorialPrompt(0)) ||
                restored.QuickBattle.Seed != 4242 || restored.SecondaryObjectives.Count != 3 || TacticalBattlefieldState.CurrentSchemaVersion != 19)
            {
                Debug.LogError("ALWAYS_FAITHFUL_LONGEVITY_REGRESSION_FAILED configuration/conditions/objectives/serialization");
                Application.Quit(1);
                yield break;
            }
            Debug.Log($"ALWAYS_FAITHFUL_LONGEVITY_REGRESSION_OK fire={fireModifier} observation={observationRange} objectives={objectives.Count}");
            Application.Quit(0);
        }

        private IEnumerator RunConcealmentMoraleRegression()
        {
            yield return null;
            ...
            for (int q = 0; q < 5; q++)
                for (int r = 0; r < 5; r++)
                {
                    ...
                    {
                        Coord = coord,
                        Terrain = TacticalTerrain.Open,
                        Cover = coord.Equals(new HexCoord(2, 2)) ? TacticalCover.Medium : TacticalCover.None,
                        OccupantId = coord.Equals(new HexCoord(2, 2)) ? "concealed" : null
                    };
                }

            var concealed = new TacticalUnitState("concealed", "Concealed Platoon", new HexCoord(2, 2), 8);
            var observer = new TacticalUnitState("observer", "Observer", new HexCoord(2, 0), 8);
            bool hid = TacticalConcealmentMorale.TryHide(concealed, board[concealed.Position], 1);
            TacticalContactState reducedContact = TacticalObservation.Check(board, observer.Id, observer.Position,
                concealed.Id, concealed.DisplayName, concealed.Position, 1, null, concealed);
            var preview = new TacticalFirePreview { IsValid = true, HitChance = 50, SuppressionChance = 60 };
            TacticalConcealmentMorale.ApplyAmbushModifier(preview, concealed);
            bool ambushApplied = preview.HitChance == 70 && preview.SuppressionChance == 75 && preview.ExpectedEffect == "AMBUSH";
            bool revealed = TacticalConcealmentMorale.Reveal(concealed);

            var searcher = new TacticalUnitState("searcher", "Search Platoon", new HexCoord(2, 1), 8);
            bool searched = TacticalConcealmentMorale.TrySearch(searcher, concealed.Position) &&
                searcher.RemainingActionPoints == 8 - TacticalConcealmentMorale.SearchActionPointCost;

            concealed.ApplySuppressionPoints(TacticalSuppression.ReducedThreshold);
            bool fallbackState = concealed.MoraleState == TacticalMoraleState.FallingBack;
            bool fallback = TacticalConcealmentMorale.TryChooseFallbackHex(board, concealed,
                new[] { observer }, out HexCoord fallbackHex) && !fallbackHex.Equals(concealed.Position);

            var battlefield = new TacticalBattlefieldState();
            battlefield.ConcealmentEvents.Add(new TacticalConcealmentEvent
            {
                Kind = TacticalConcealmentEventKind.Ambush,
                UnitId = concealed.Id,
                Origin = concealed.Position,
                Destination = concealed.Position
            });
            TacticalBattlefieldState restored = JsonUtility.FromJson<TacticalBattlefieldState>(JsonUtility.ToJson(battlefield));
            if (!hid || reducedContact.State >= TacticalVisibilityState.Observed || !ambushApplied || !revealed ||
                !searched || !fallbackState || !fallback || restored == null || restored.ConcealmentEvents.Count != 1 ||
                TacticalBattlefieldState.CurrentSchemaVersion != 19)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_CONCEALMENT_MORALE_REGRESSION_FAILED hide={hid} contact={reducedContact.State} ambush={ambushApplied} reveal={revealed} search={searched} fallback={fallbackState}/{fallback} schema={restored?.SchemaVersion}");
                Application.Quit(1);
                yield break;
            }
            Debug.Log($"ALWAYS_FAITHFUL_CONCEALMENT_MORALE_REGRESSION_OK contact={reducedContact.State} ambush={preview.HitChance}/{preview.SuppressionChance} fallback={fallbackHex}");
            Application.Quit(0);
        }

        private IEnumerator RunObstaclesRegression()
        {
            yield return null;
            var origin = new HexCoord(0, 0);
            ...
            var board = new Dictionary<HexCoord, TacticalMovementCell>
            {
                [origin] = new TacticalMovementCell { Coord = origin, Terrain = TacticalTerrain.Open, OccupantId = "usmc-test" },
                [destination] = new TacticalMovementCell
                {
                    Coord = destination,
                    Terrain = TacticalTerrain.Open,
                    Obstacle = TacticalObstacleType.Minefield,
                    ObstacleIntelligence = TacticalObstacleIntelligence.Hidden
                }
            };
            TacticalRouteResult hiddenRoute = TacticalMovementPlanner.FindRoute(board, origin, destination, 8, "usmc-test");
            board[destination].ObstacleIntelligence = TacticalObstacleIntelligence.Identified;
            TacticalRouteResult knownRoute = TacticalMovementPlanner.FindRoute(board, origin, destination, 8, "usmc-test");
            board[destination].ObstacleOwnerSide = "USMC";
            TacticalRouteResult friendlyDefenseRoute = TacticalMovementPlanner.FindRoute(board, origin, destination, 8, "imported-friendly", "USMC");
            TacticalRouteResult hostileDefenseRoute = TacticalMovementPlanner.FindRoute(board, origin, destination, 8, "imported-enemy", "PLA");
            board[destination].ObstacleOwnerSide = null;

            var obstacle = new TacticalObstacleState
            {
                Id = "OBS-TEST",
                Position = destination,
                Type = TacticalObstacleType.Minefield,
                OwnerSide = "PLA"
            };
            TacticalObstacleEncounter quick = TacticalObstacles.Enter(obstacle, TacticalMovementPosture.Quick, "USMC", 77);
            TacticalObstacleEncounter owner = TacticalObstacles.Enter(obstacle, TacticalMovementPosture.Tactical, "PLA", 78);
            obstacle.IsBreached = true;
            int breachedCost = TacticalObstacles.MovementCost(obstacle.Type, obstacle.UsmcIntelligence, obstacle.IsBreached);
            var intelligenceProbe = new TacticalObstacleState { Type = TacticalObstacleType.Wire };
            TacticalObstacleIntelligence suspected = TacticalObstacles.ImproveIntelligence(
                intelligenceProbe, TacticalObstacleIntelligence.Suspected);
            TacticalObstacleIntelligence detected = TacticalObstacles.ImproveIntelligence(
                intelligenceProbe, TacticalObstacleIntelligence.Detected);
            TacticalObstacleIntelligence noDowngrade = TacticalObstacles.ImproveIntelligence(
                intelligenceProbe, TacticalObstacleIntelligence.Suspected);
            var engineer = new TacticalUnitState("usmc-engineers", "Engineer Probe", origin, 8)
            {
                MaximumEngineerSupply = TacticalObstacles.DefaultEngineerSupply,
                EngineerSupply = TacticalObstacles.DefaultEngineerSupply
            };
            var breachTarget = new TacticalObstacleState
            {
                Id = "BREACH-TEST",
                Position = destination,
                Type = TacticalObstacleType.Wire,
                OwnerSide = "PLA"
            };
            bool detectedByEngineer = TacticalObstacles.TryDetect(engineer, breachTarget, destination, true) &&
                breachTarget.UsmcIntelligence == TacticalObstacleIntelligence.Identified;
            bool breachIssued = TacticalObstacles.TryBreach(engineer, breachTarget, true, true, 91, out bool breachSucceeded);
            var breachedCell = new TacticalMovementCell { Coord = destination, Terrain = TacticalTerrain.Open };
            TacticalObstacles.ApplyToCell(breachedCell, breachTarget);
            bool safeLane = breachIssued && breachSucceeded && breachTarget.IsBreached &&
                engineer.RemainingActionPoints == 2 && engineer.EngineerSupply == 3 &&
                TacticalObstacles.MovementCost(breachedCell.Obstacle, breachedCell.ObstacleIntelligence, breachedCell.ObstacleBreached) == 0;
            var fortifiedDefender = new TacticalUnitState("fortified", "Fortified", destination, 8) { EntrenchmentLevel = 1 };
            int normalAssault = TacticalFireAndManeuver.AssaultChance(engineer, fortifiedDefender, breachedCell);
            int engineerAssault = TacticalFireAndManeuver.AssaultChance(engineer, fortifiedDefender, breachedCell, true);
            bool engineerReduction = engineerAssault == normalAssault + 20;
            List<TacticalSecondaryObjectiveState> breachObjectives = TacticalStandaloneRules.CreateSecondaryObjectives(TacticalMissionType.Attack, true);

            var battlefield = new TacticalBattlefieldState
            {
                BattlefieldId = "OBSTACLE-REGRESSION",
                Seed = 2718,
                Width = 9,
                Height = 9
            };
            for (int q = 0; q < 9; q++)
                for (int r = 0; r < 9; r++)
                    ...
                    {
                        LocalCoord = new HexCoord(q, r),
                        Terrain = TacticalTerrain.Open
                    });
            List<TacticalObstacleState> layout = TacticalObstacles.GenerateDefensiveLayout(
                battlefield, new HexCoord(4, 4), new HashSet<HexCoord> { new HexCoord(4, 2) });
            battlefield.Obstacles.AddRange(layout);
            battlefield.ObstacleEvents.Add(new TacticalObstacleEvent
            {
                Kind = TacticalObstacleEventKind.MovementHalted,
                ObstacleId = obstacle.Id,
                Position = obstacle.Position,
                Type = obstacle.Type
            });
            TacticalBattlefieldState restored = JsonUtility.FromJson<TacticalBattlefieldState>(JsonUtility.ToJson(battlefield));
            bool typesPresent = layout.Exists(item => item.Type == TacticalObstacleType.Minefield) &&
                layout.Exists(item => item.Type == TacticalObstacleType.Wire) &&
                layout.Exists(item => item.Type == TacticalObstacleType.Roadblock);
            if (!hiddenRoute.IsValid || hiddenRoute.ActionPointCost != 1 || !knownRoute.IsValid || knownRoute.ActionPointCost != 3 ||
                friendlyDefenseRoute.ActionPointCost != 1 || hostileDefenseRoute.ActionPointCost != 3 ||
                !quick.Halted || !quick.NewlyIdentified || quick.Suppression != 3 || quick.StrengthDamage < 8 || owner.Halted ||
                breachedCost != 0 || suspected != TacticalObstacleIntelligence.Suspected ||
                detected != TacticalObstacleIntelligence.Detected || noDowngrade != TacticalObstacleIntelligence.Detected ||
                !detectedByEngineer || !safeLane || !engineerReduction || !breachObjectives.Exists(item => item.Type == TacticalSecondaryObjectiveType.BreachRoute) ||
                layout.Count != TacticalObstacles.DefaultObstacleCount || !typesPresent ||
                restored == null || restored.SchemaVersion != 19 || restored.Obstacles.Count != layout.Count || restored.ObstacleEvents.Count != 1)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_OBSTACLES_REGRESSION_FAILED routes={hiddenRoute.ActionPointCost}/{knownRoute.ActionPointCost} quick={quick.Halted}/{quick.Suppression}/{quick.StrengthDamage} owner={owner.Halted} breached={breachedCost} layout={layout.Count}/{typesPresent} schema={restored?.SchemaVersion}");
                Application.Quit(1);
                yield break;
            }
            Debug.Log($"ALWAYS_FAITHFUL_OBSTACLES_REGRESSION_OK hiddenCost={hiddenRoute.ActionPointCost} knownCost={knownRoute.ActionPointCost} damage={quick.StrengthDamage} layout={layout.Count} schema={restored.SchemaVersion}");
            Application.Quit(0);
        }

        private IEnumerator RunPlannedFiresRegression()
        {
            yield return null;
            var board = new Dictionary<HexCoord, TacticalMovementCell>();
            for (int q = 0; q < 7; q++)
                ...
                {
                    var coord = new HexCoord(q, r);
                    board[coord] = new TacticalMovementCell { Coord = coord, Terrain = TacticalTerrain.Open };
                }
            var observer = new TacticalUnitState("observer", "Recon Observer", new HexCoord(1, 1), 8);
            var support = new TacticalFireSupportState();
            HexCoord target = new HexCoord(4, 4);
            TacticalPlannedFireMission mission = TacticalPlannedFires.Order(observer, support,
                TacticalFireMissionType.HighExplosive, target, 1, TacticalVisibilityState.Contact,
                true, false, 1);
            bool delayed = mission != null && mission.Status == TacticalFireMissionStatus.Pending &&
                mission.DeliveryTurn == 2 && observer.RemainingActionPoints == 5 && support.Ammunition == 3 &&
                support.NextAvailableTurn == 3 && mission.FinalScatterRadius == 1;
            bool adjusted = TacticalPlannedFires.TryAdjust(observer, support, target) &&
                TacticalPlannedFires.ScatterRadius(TacticalVisibilityState.Contact, true, support.AdjustmentLevel) == 0;
            HexCoord impactA = TacticalPlannedFires.ResolveScatter(board, target, 2, 84);
            HexCoord impactB = TacticalPlannedFires.ResolveScatter(board, target, 2, 84);
            var weapons = new TacticalUnitState("weapons", "Weapons Platoon", new HexCoord(2, 2), 8)
            {
                MaximumMortarRounds = TacticalPlannedFires.DefaultMortarAmmunition,
                MortarRounds = TacticalPlannedFires.DefaultMortarAmmunition
            };
            bool mortarSetup = TacticalPlannedFires.TrySetupMortar(weapons, true);
            TacticalPlannedFireMission mortar = TacticalPlannedFires.Order(weapons, support,
                TacticalFireMissionType.MortarSmoke, new HexCoord(5, 2), 1, TacticalVisibilityState.Hidden,
                false, false, 2);
            TacticalFireAndManeuver.RecordMove(weapons, TacticalMovementPosture.Tactical,
                new List<HexCoord> { new HexCoord(2, 2), new HexCoord(2, 3) });
            bool displacement = mortarSetup && mortar != null && weapons.MortarRounds == 3 && !weapons.MortarIsDeployed;
            bool dangerClose = TacticalPlannedFires.IsDangerClose(new[]
                { new TacticalUnitState("friendly", "Friendly", new HexCoord(4, 3), 8) }, target);
            var battlefield = new TacticalBattlefieldState();
            battlefield.PlannedFireMissions.Add(mission);
            battlefield.ActiveIlluminationMarkers.Add(new TacticalIlluminationMarker
                { Position = target, PlacedTurn = 2, ExpiresAfterTurn = 3 });
            battlefield.PlannedFireEvents.Add(new TacticalPlannedFireEvent { MissionId = mission.Id, Type = mission.Type });
            TacticalBattlefieldState restored = JsonUtility.FromJson<TacticalBattlefieldState>(JsonUtility.ToJson(battlefield));
            bool persistent = restored != null && restored.SchemaVersion == 19 && restored.PlannedFireMissions.Count == 1 &&
                restored.PlannedFireEvents.Count == 1 && TacticalPlannedFires.IsIlluminated(restored.ActiveIlluminationMarkers, target, 3);
            if (!delayed || !adjusted || !impactA.Equals(impactB) || !displacement || !dangerClose || !persistent)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_PLANNED_FIRES_REGRESSION_FAILED delayed={delayed} adjusted={adjusted} scatter={impactA}/{impactB} mortar={displacement} danger={dangerClose} persistent={persistent}");
                Application.Quit(1);
                yield break;
            }
            Debug.Log($"ALWAYS_FAITHFUL_PLANNED_FIRES_REGRESSION_OK delivery={mission.DeliveryTurn} scatter={mission.FinalScatterRadius} mortar={weapons.MortarRounds} schema={restored.SchemaVersion}");
            Application.Quit(0);
        }

        private IEnumerator RunFireManeuverRegression()
        {
            yield return null;
            var board = new Dictionary<HexCoord, TacticalMovementCell>();
            for (int q = 0; q < 4; q++)
                ...
                {
                    HexCoord coord = new HexCoord(q, r);
                    board[coord] = new TacticalMovementCell { Coord = coord, Terrain = TacticalTerrain.Open, ElevationMetres = 10f };
                }

            if (TacticalFireAndManeuver.PlanningBudget(8, TacticalMovementPosture.Quick) != 16 ||
                TacticalFireAndManeuver.PlanningBudget(8, TacticalMovementPosture.Tactical) != 8 ||
                TacticalFireAndManeuver.PlanningBudget(8, TacticalMovementPosture.Bounding) != 5 ||
                TacticalFireAndManeuver.MovementCost(6, TacticalMovementPosture.Quick) != 3 ||
                TacticalFireAndManeuver.MovementCost(6, TacticalMovementPosture.Bounding) != 9)
            {
                Debug.LogError("ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_FAILED movement posture economy");
                Application.Quit(1);
                yield break;
            }

            var reactor = new TacticalUnitState("reactor", "Reactor", new HexCoord(0, 0), 8);
            HexCoord forward = new HexCoord(0, 1);
            TacticalFireAndManeuver.Face(reactor, forward);
            if (TacticalFireAndManeuver.FacingAspect(reactor, forward) != TacticalFacingAspect.Forward)
            {
                Debug.LogError("ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_FAILED facing sector");
                Application.Quit(1);
                yield break;
            }
            reactor.ReactionPolicy = TacticalReactionPolicy.WeaponsHold;
            bool hold = TacticalFireAndManeuver.CanReact(reactor);
            reactor.ReactionPolicy = TacticalReactionPolicy.ReturnFire;
            bool returnBeforeFire = TacticalFireAndManeuver.CanReact(reactor);
            reactor.WasFiredUponThisTurn = true;
            bool returnAfterFire = TacticalFireAndManeuver.CanReact(reactor);
            reactor.ReactionPolicy = TacticalReactionPolicy.WeaponsFree;
            reactor.ReactionPoints = 0;
            bool depleted = TacticalFireAndManeuver.CanReact(reactor);
            if (hold || returnBeforeFire || !returnAfterFire || depleted)
            {
                Debug.LogError("ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_FAILED reaction policy/reserve");
                Application.Quit(1);
                yield break;
            }

            board[new HexCoord(0, 1)].SmokeExpiresAfterTurn = 3;
            TacticalLosResult smokeLos = TacticalLineOfSight.Inspect(board, new HexCoord(0, 0), new HexCoord(0, 2));
            if (!smokeLos.IsValid || smokeLos.State != TacticalLosState.Obscured || TacticalFireAndManeuver.AreaSuppressionChance(smokeLos) <= 0)
            {
                Debug.LogError("ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_FAILED smoke/suppression LOS");
                Application.Quit(1);
                yield break;
            }

            var attacker = new TacticalUnitState("attacker", "Attacker", new HexCoord(1, 1), 8);
            var defender = new TacticalUnitState("defender", "Defender", new HexCoord(1, 2), 8);
            int baseAssault = TacticalFireAndManeuver.AssaultChance(attacker, defender, board[new HexCoord(1, 2)]);
            defender.ApplySuppressionPoints(3);
            int suppressedAssault = TacticalFireAndManeuver.AssaultChance(attacker, defender, board[new HexCoord(1, 2)]);
            board[new HexCoord(1, 2)].Cover = TacticalCover.Heavy;
            int coveredAssault = TacticalFireAndManeuver.AssaultChance(attacker, defender, board[new HexCoord(1, 2)]);
            if (suppressedAssault <= baseAssault || coveredAssault >= suppressedAssault)
            {
                Debug.LogError("ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_FAILED assault suppression/cover balance");
                Application.Quit(1);
                yield break;
            }

            attacker.ReactionPoints = 0;
            attacker.MovedThisTurn = attacker.FiredThisTurn = attacker.WasFiredUponThisTurn = true;
            attacker.BeginTurn();
            var battlefieldProbe = new TacticalBattlefieldState();
            battlefieldProbe.ActiveSmokeMarkers.Add(new TacticalSmokeMarker { Position = new HexCoord(0, 1), PlacedTurn = 1, ExpiresAfterTurn = 2, SourceUnitId = attacker.Id });
            string json = JsonUtility.ToJson(battlefieldProbe);
            TacticalBattlefieldState roundTrip = JsonUtility.FromJson<TacticalBattlefieldState>(json);
            if (attacker.ReactionPoints != TacticalFireAndManeuver.ReactionPointMaximum || attacker.MovedThisTurn || attacker.FiredThisTurn || attacker.WasFiredUponThisTurn ||
                TacticalBattlefieldState.CurrentSchemaVersion != 19 || roundTrip.ActiveSmokeMarkers.Count != 1)
            {
                Debug.LogError("ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_FAILED turn reset/schema serialization");
                Application.Quit(1);
                yield break;
            }

            Debug.Log($"ALWAYS_FAITHFUL_FIRE_MANEUVER_REGRESSION_OK baseAssault={baseAssault} suppressed={suppressedAssault} covered={coveredAssault} smoke={smokeLos.State}");
            Application.Quit(0);
        }
        ...
        private static bool ValidateCoverRules(out string failure)
        {
            ...
            var noneReactor = new TacticalUnitState("reactor-none", "Reactor", attackerPosition, 4);
            var heavyReactor = new TacticalUnitState("reactor-heavy", "Reactor", attackerPosition, 4);
            TacticalFireAndManeuver.Face(noneReactor, noneTargetPosition);
            TacticalFireAndManeuver.Face(heavyReactor, heavyTargetPosition);
            ...
            TacticalFirePreview noneReaction = TacticalReactionFire.Preview(board, noneReactor, weapon, noneTargetPosition, losToNone);
            TacticalFirePreview heavyReaction = TacticalReactionFire.Preview(board, heavyReactor, weapon, heavyTargetPosition, losToHeavy);
        ...
        private IEnumerator RunVictoryRegression()
        {
            ...
            int mergedEventCount = tacticalBattlefield.MovementEvents.Count + tacticalBattlefield.FireEvents.Count +
                ...
                tacticalBattlefield.ReconEvents.Count + tacticalBattlefield.ObjectiveEvents.Count + tacticalBattlefield.SupportCardEvents.Count +
                tacticalBattlefield.SmokeEvents.Count + tacticalBattlefield.AreaSuppressionEvents.Count + tacticalBattlefield.AssaultEvents.Count +
                tacticalBattlefield.StrengthEvents.Count + tacticalBattlefield.PositionEvents.Count + tacticalBattlefield.LogisticsEvents.Count +
                tacticalBattlefield.CallForFireEvents.Count +
        ...
        {
            if (TacticalBattlefieldState.CurrentSchemaVersion != 19)
            {
                failure = $"expected schema version 19, got {TacticalBattlefieldState.CurrentSchemaVersion}";
                return false;
            }
        ...
        private static bool ValidateScenarioRules(out string failure)
        {
            if (TacticalBattlefieldState.CurrentSchemaVersion != 19)
            {
                failure = $"expected schema version 19 after adding planned-fire state and history, got {TacticalBattlefieldState.CurrentSchemaVersion}";
                return false;
            }
        ...
        private IEnumerator RunCompanyRegression()
        {
            yield return null;
            List<TacticalPlatoonDefinition> balanced = TacticalCompany.BuildRoster(TacticalCompanyPreset.Balanced);
            List<TacticalPlatoonDefinition> recon = TacticalCompany.BuildRoster(TacticalCompanyPreset.Recon);
            List<TacticalPlatoonDefinition> assault = TacticalCompany.BuildRoster(TacticalCompanyPreset.Assault);
            if (balanced.Count != TacticalCompany.DefaultPlatoonCount || recon.Count != TacticalCompany.DefaultPlatoonCount ||
                assault.Count != TacticalCompany.DefaultPlatoonCount || balanced[3].Role != TacticalPlatoonRole.Weapons ||
                recon[3].Role != TacticalPlatoonRole.Reconnaissance || assault[3].Role != TacticalPlatoonRole.Engineers ||
                !MiniatureCounterView.HasModelsFor(TacticalPlatoonRole.Weapons) ||
                !MiniatureCounterView.HasModelsFor(TacticalPlatoonRole.Engineers) ||
                !MiniatureCounterView.HasModelsFor(TacticalPlatoonRole.Reconnaissance))
            {
                Debug.LogError("ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED roster presets");
                Application.Quit(1);
                yield break;
            }
            ...
            EnterTacticalMap(FindHighReliefOperationalCell(), false);
            var positions = new HashSet<HexCoord>();
            bool occupancyOk = tacticalFriendlyStates.Count == TacticalCompany.DefaultPlatoonCount &&
                tacticalFriendlyViews.Count == tacticalFriendlyStates.Count && tacticalFriendlyWeapons.Count == tacticalFriendlyStates.Count;
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
            {
                if (!positions.Add(friendly.Position) || localMovementBoard[friendly.Position].OccupantId != friendly.Id) occupancyOk = false;
            }
            if (!occupancyOk || tacticalEnemyStates.Count < TacticalScenario.MinRosterSize)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED initial force friendlies={tacticalFriendlyStates.Count} views={tacticalFriendlyViews.Count} weapons={tacticalFriendlyWeapons.Count} enemies={tacticalEnemyStates.Count} occupancy={occupancyOk}");
                Application.Quit(1);
                yield break;
            }

            string firstSelected = tacticalUnitState.Id;
            CycleTacticalFriendly(false);
            if (tacticalUnitState.Id == firstSelected)
            {
                Debug.LogError("ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED unit cycling did not advance");
                Application.Quit(1);
                yield break;
            }

            BeginTacticalDeployment(TacticalCompanyPreset.Recon);
            bool hasRecon = tacticalFriendlyStates.Exists(candidate => InferFriendlyRole(candidate.Id) == TacticalPlatoonRole.Reconnaissance);
            HexCoord center = new HexCoord(tacticalBattlefield.Width / 2, tacticalBattlefield.Height / 2);
            bool inZone = tacticalFriendlyStates.TrueForAll(candidate => HexCoord.Distance(center, candidate.Position) <= 3);
            if (!hasRecon || !inZone || !tacticalDeploymentActive)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED force/deployment selection recon={hasRecon} inZone={inZone} active={tacticalDeploymentActive}");
                Application.Quit(1);
                yield break;
            }
            ConfirmTacticalDeployment();

            TacticalUnitState nonActive = tacticalFriendlyStates.Find(candidate => candidate != tacticalUnitState);
            TacticalMovementCell objectiveCell = localMovementBoard[tacticalObjective.ObjectiveHex];
            string originalOccupant = objectiveCell.OccupantId;
            objectiveCell.OccupantId = nonActive.Id;
            bool companyControlsObjective = ObjectiveControlledByUsmc();
            objectiveCell.OccupantId = originalOccupant;
            if (!companyControlsObjective)
            {
                Debug.LogError("ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED non-active platoon did not control objective");
                Application.Quit(1);
                yield break;
            }

            TacticalUnitState aiTarget = FindBestTargetForEnemy(tacticalEnemyStates[0], null, out TacticalContactState aiContact);
            if (aiTarget == null || aiContact == null || FindFriendly(aiTarget.Id) == null)
            {
                Debug.LogError("ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED AI target selection");
                Application.Quit(1);
                yield break;
            }

            string savePath = Path.Combine(Path.GetTempPath(), "always-faithful-company-regression.json");
            SaveTacticalBattle(savePath);
            TacticalBattleSaveState saved = JsonUtility.FromJson<TacticalBattleSaveState>(File.ReadAllText(savePath));
            if (saved.SchemaVersion != TacticalBattleSaveState.CurrentSchemaVersion || saved.FriendlyUnits.Count != TacticalCompany.DefaultPlatoonCount ||
                saved.FriendlyWeapons.Count != TacticalCompany.DefaultPlatoonCount || saved.SelectedFriendlyUnitId != tacticalUnitState.Id ||
                saved.CompanyPreset != TacticalCompanyPreset.Recon)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_COMPANY_REGRESSION_FAILED save schema={saved?.SchemaVersion} units={saved?.FriendlyUnits?.Count} weapons={saved?.FriendlyWeapons?.Count} selected={saved?.SelectedFriendlyUnitId} preset={saved?.CompanyPreset}");
                Application.Quit(1);
                yield break;
            }

            Debug.Log($"ALWAYS_FAITHFUL_COMPANY_REGRESSION_OK friendlies={tacticalFriendlyStates.Count} enemies={tacticalEnemyStates.Count} preset={tacticalCompanyPreset} target={aiTarget.Id} contact={aiContact.State}");
            Application.Quit(0);
        }
        ...
        private static bool ValidateSettingsRules(out string failure)
        {
            ...
            var reservedHudKey = AlwaysFaithfulSettingsRules.CreateDefault();
            reservedHudKey.RemapResetCameraKey = KeyCode.H;
            if (AlwaysFaithfulSettingsRules.Validate(reservedHudKey, out _))
            {
                failure = "the reserved HUD key was accepted as a remappable action";
                return false;
            }
        ...
        private bool TryManufactureLostContact(TacticalUnitState targetEnemy, out TacticalUnitState enemy)
        {
            ...
            var distantHexes = new List<HexCoord>();
            foreach (KeyValuePair<HexCoord, TacticalMovementCell> pair in localMovementBoard)
                if (pair.Value.IsPassable && HexCoord.Distance(pair.Key, targetEnemy.Position) > TacticalObservation.ObscuredContactRangeHexes)
                    distantHexes.Add(pair.Key);
            distantHexes.Sort((a, b) =>
            {
                int byDistance = HexCoord.Distance(b, targetEnemy.Position).CompareTo(HexCoord.Distance(a, targetEnemy.Position));
                if (byDistance != 0) return byDistance;
                int byQ = a.Q.CompareTo(b.Q);
                return byQ != 0 ? byQ : a.R.CompareTo(b.R);
            });
            if (distantHexes.Count < tacticalFriendlyStates.Count) return false;
            foreach (TacticalUnitState friendly in tacticalFriendlyStates)
                localMovementBoard[friendly.Position].OccupantId = null;
            for (int index = 0; index < tacticalFriendlyStates.Count; index++)
            {
                TacticalUnitState friendly = tacticalFriendlyStates[index];
                friendly.Position = distantHexes[index];
                localMovementBoard[friendly.Position].OccupantId = friendly.Id;
                if (tacticalFriendlyViews.TryGetValue(friendly.Id, out TacticalFriendlyView friendlyView))
                    friendlyView.Counter.transform.position = LocalCounterPosition(friendly.Position);
            }
            farHex = tacticalUnitState.Position;
        ...
        private IEnumerator RunOperationalScenarioRegression()
        {
            ...
            if (!OperationalScenarioRules.Validate(operationalScenario, out string validationError) ||
                ...
                operationalFriendlyBattalions.Exists(battalion => battalion.Platoons.Count != 12) ||
                operationalEnemyBattalions.Exists(battalion => battalion.Configuration == OperationalBattalionConfiguration.PlaAmphibiousCombinedArms && battalion.Platoons.Count != 12) ||
                operationalEnemyBattalions.Exists(battalion => battalion.Configuration == OperationalBattalionConfiguration.PlaReconnaissance && battalion.Platoons.Count != 11))
            {
            ...
            tacticalFriendlyStates[0].Strength = 73;
            ApplyOperationalBattleLosses();
            if (activeOperationalBattalion.Platoons[0].Strength != 73 || activeOperationalBattalion.Strength >= 100)
            {
                Debug.LogError($"ALWAYS_FAITHFUL_OPERATIONAL_SCENARIO_REGRESSION_FAILED one-for-one losses platoon={activeOperationalBattalion.Platoons[0].Strength} battalion={activeOperationalBattalion.Strength}");
                Application.Quit(1);
                yield break;
            }
        ...
        private void ApplyCamera()
        {
            ...
            if (tacticalMode)
            {
                ...
                foreach (TacticalFriendlyView friendly in tacticalFriendlyViews.Values)
                    if (friendly.Counter != null) friendly.Counter.transform.localScale = Vector3.one * Mathf.Clamp(cameraDistance / 30f, .82f, 1.55f);
                return;
            }
            mapCamera.transform.position = cameraPosition;
            mapCamera.transform.LookAt(lookTarget, Vector3.up);
        ...
        private void OnGUI()
        {
            ...
            hoveredOrderTooltip = null;

            if (tacticalMode)
            {
                if (!hudCollapsed) DrawTacticalInterface(uiWidth, uiHeight);
                else
                {
                    DrawCollapsedHudTabs(uiWidth, uiHeight);
                    DrawTacticalInterface(uiWidth, uiHeight, true);
                }
                DrawTacticalCenterNotices(uiWidth, uiHeight);
                ...
                if (!hudCollapsed && !tacticalForcePanelOpen && !tacticalDeploymentActive)
                {
                ...
                DrawTacticalDeploymentControls(uiWidth, uiHeight);
                DrawTacticalForcePanel(uiWidth, uiHeight);
                DrawOrderTooltip(uiWidth, uiHeight);
                ...
                if (inspected.IsLand && selectedCell == inspected)
                {
                    ...
                    if (resolvingBattalion != null)
                    {
                        ...
                        if (GUI.Button(enterTacticalRect, tacticalLabel, buttonStyle))
                            OpenOperationalTacticalMap(inspected, resolvingBattalion, inspectedContact);
                    }
            ...
            if (counterMenuOpen)
            {
                ...
                if (DrawOrderButton(new Rect(counterMenuRect.x + 10f, counterMenuRect.y + 34f, 158f, 28f), moveLabel,
                        "MOVE | Plot a terrain-costed route for this battalion. The order spends operational AP when you confirm the destination."))
                    BeginMovePlanning();
                GUI.enabled = unitState.CanMove && unitState.RemainingActionPoints >= OperationalScenarioRules.ReconActionPointCost;
                if (DrawOrderButton(new Rect(counterMenuRect.x + 10f, counterMenuRect.y + 66f, 158f, 28f),
                        $"RECON  •  {OperationalScenarioRules.ReconActionPointCost} AP",
                        "RECON | Search a selected area for two operational turns. It can reveal, identify, or refine otherwise hidden PLA battalion contacts."))
                    BeginOperationalReconPlanning();
                GUI.enabled = true;
            }
            DrawTransitionOverlay(uiWidth, uiHeight);
        ...
        private void DrawOperationalUnitMenu()
        {
            if (!counterMenuOpen || activeOperationalBattalion == null || unitState == null) return;
            GUI.Box(counterMenuRect, GUIContent.none);
            GUI.Label(new Rect(counterMenuRect.x + 12f, counterMenuRect.y + 7f, 154f, 22f),
                activeOperationalBattalion.ShortName + " | INF BN", badgeStyle);
            GUI.enabled = unitState.CanMove;
            string moveLabel = unitState.CanMove ? $"MOVE | {unitState.RemainingActionPoints} AP" : "MOVE | SPENT";
            if (DrawOrderButton(new Rect(counterMenuRect.x + 10f, counterMenuRect.y + 34f, 158f, 28f), moveLabel,
                    "MOVE | Plot a terrain-costed route for this battalion. The order spends operational AP when you confirm the destination."))
        ...
        private bool DrawOrderButton(Rect rect, string label, string explanation)
        {
            Vector2 pointer = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y) / GetUiScale();
            if (rect.Contains(pointer)) hoveredOrderTooltip = explanation;
            return GUI.Button(rect, new GUIContent(label, explanation), buttonStyle);
        }

        private void DrawOrderTooltip(float uiWidth, float uiHeight)
        {
            if (string.IsNullOrWhiteSpace(hoveredOrderTooltip)) return;
            const float width = 350f;
            float textHeight = bodyStyle.CalcHeight(new GUIContent(hoveredOrderTooltip), width - 24f);
            float height = Mathf.Max(58f, textHeight + 24f);
            Vector2 pointer = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y) / GetUiScale();
            float x = pointer.x + 18f;
            if (x + width > uiWidth - 8f) x = pointer.x - width - 18f;
            float y = Mathf.Clamp(pointer.y + 18f, 8f, uiHeight - height - 8f);
            Rect box = new Rect(Mathf.Clamp(x, 8f, uiWidth - width - 8f), y, width, height);
            GUI.Box(box, GUIContent.none);
            GUI.Label(new Rect(box.x + 12f, box.y + 10f, box.width - 24f, box.height - 20f), hoveredOrderTooltip, bodyStyle);
        }

        private void DrawCollapsedHudTabs(float uiWidth, float uiHeight)
        {
            GUIStyle edgeStyle = new GUIStyle(buttonStyle)
            {
                alignment = TextAnchor.MiddleCenter,
                fontSize = 12,
                wordWrap = true
            };
            float y = uiHeight * .5f - 54f;
            if (GUI.Button(new Rect(-2f, y, 34f, 108f), "HUD\nH\n›", edgeStyle) ||
                GUI.Button(new Rect(uiWidth - 32f, y, 34f, 108f), "‹\nH\nHUD", edgeStyle))
                ToggleHudVisibility();
        }

        private void DrawTacticalForcePanel(float uiWidth, float uiHeight)
        {
            if (!tacticalForcePanelOpen) return;
            Color previous = GUI.color;
            GUI.color = new Color(.02f, .035f, .04f, .84f);
            GUI.DrawTexture(new Rect(0f, 0f, uiWidth, uiHeight), Texture2D.whiteTexture);
            GUI.color = previous;
            Rect panel = new Rect(uiWidth / 2f - 360f, uiHeight / 2f - 310f, 720f, 620f);
            GUI.Box(panel, GUIContent.none);
            GUI.Label(new Rect(panel.x + 24f, panel.y + 20f, panel.width - 48f, 34f), "ORGANIZE THE COMPANY", titleStyle);
            GUI.Label(new Rect(panel.x + 24f, panel.y + 62f, panel.width - 48f, 54f),
                "Every counter is a platoon. Choose one attachment for the three-platoon rifle core, then deploy the company inside the marked zone.", bodyStyle);
            DrawCompanyPresetButton(new Rect(panel.x + 24f, panel.y + 132f, 208f, 250f), TacticalCompanyPreset.Balanced,
                "BALANCED", "WEAPONS PLATOON", "Eight ammunition units for sustained supporting fire.");
            DrawCompanyPresetButton(new Rect(panel.x + 256f, panel.y + 132f, 208f, 250f), TacticalCompanyPreset.Recon,
                "RECON", "RECON PLATOON", "Ten AP and one-point-cheaper Recon tasking.");
            DrawCompanyPresetButton(new Rect(panel.x + 488f, panel.y + 132f, 208f, 250f), TacticalCompanyPreset.Assault,
                "ASSAULT", "ENGINEER PLATOON", "Nine AP now; breaching and fieldworks arrive with their rule package.");
            GUI.Label(new Rect(panel.x + 24f, panel.y + 410f, panel.width - 48f, 28f),
                "Core: 1st, 2nd, and 3rd Rifle Platoons • Attachment: your selection", badgeStyle);
            TacticalQuickBattleConfiguration quick = tacticalBattlefield.QuickBattle;
            GUI.Label(new Rect(panel.x + 24f, panel.y + 446f, panel.width - 48f, 24f), "QUICK BATTLE CONDITIONS", badgeStyle);
            if (GUI.Button(new Rect(panel.x + 24f, panel.y + 476f, 158f, 34f), $"MISSION • {quick.MissionType}", buttonStyle)) CycleQuickMission();
            if (GUI.Button(new Rect(panel.x + 194f, panel.y + 476f, 158f, 34f), $"WEATHER • {quick.Weather}", buttonStyle)) CycleQuickWeather();
            if (GUI.Button(new Rect(panel.x + 364f, panel.y + 476f, 158f, 34f), $"LIGHT • {quick.Visibility}", buttonStyle)) CycleQuickVisibility();
            if (GUI.Button(new Rect(panel.x + 534f, panel.y + 476f, 162f, 34f), $"ENEMY • {quick.Difficulty}", buttonStyle)) CycleQuickDifficulty();
            if (GUI.Button(new Rect(panel.x + 24f, panel.y + 522f, 216f, 34f), $"TURN LIMIT • {quick.TurnLimit}", buttonStyle))
                quick.TurnLimit = quick.TurnLimit >= 14 ? 6 : quick.TurnLimit + 2;
            if (GUI.Button(new Rect(panel.x + 252f, panel.y + 522f, 216f, 34f), quick.TutorialEnabled ? "TUTORIAL • ON" : "TUTORIAL • OFF", buttonStyle))
                quick.TutorialEnabled = !quick.TutorialEnabled;
            GUI.Label(new Rect(panel.x + 480f, panel.y + 522f, 216f, 52f),
                "Conditions affect detection and fire. Settings persist in the battle save.", bodyStyle);
        }

        private void DrawCompanyPresetButton(Rect rect, TacticalCompanyPreset preset, string title, string attachment, string description)
        {
            GUI.Box(rect, GUIContent.none);
            GUI.Label(new Rect(rect.x + 14f, rect.y + 16f, rect.width - 28f, 28f), title, unitNameStyle);
            GUI.Label(new Rect(rect.x + 14f, rect.y + 60f, rect.width - 28f, 24f), attachment, badgeStyle);
            GUI.Label(new Rect(rect.x + 14f, rect.y + 98f, rect.width - 28f, 82f), description, bodyStyle);
            if (GUI.Button(new Rect(rect.x + 14f, rect.y + rect.height - 52f, rect.width - 28f, 36f), "SELECT", buttonStyle))
                BeginTacticalDeployment(preset);
        }
        ...
        private void DrawTacticalDeploymentControls(float uiWidth, float uiHeight)
        {
            ...
            if (tacticalDefensiveObstacleBudget > 0)
            {
                if (GUI.Button(new Rect(panel.x + 18f, panel.y + 86f, 170f, 30f),
                        tacticalObstaclePlacementActive ? "CANCEL DEFENSE" : $"PLACE DEFENSE • {tacticalDefensiveObstacleBudget}", buttonStyle))
                    tacticalObstaclePlacementActive = !tacticalObstaclePlacementActive;
                if (GUI.Button(new Rect(panel.x + 194f, panel.y + 86f, 142f, 30f), tacticalDefensiveObstacleType.ToString().ToUpperInvariant(), buttonStyle))
                    tacticalDefensiveObstacleType = tacticalDefensiveObstacleType == TacticalObstacleType.Minefield
                        ? TacticalObstacleType.Wire : tacticalDefensiveObstacleType == TacticalObstacleType.Wire
                            ? TacticalObstacleType.Roadblock : TacticalObstacleType.Minefield;
            }
            if (GUI.Button(new Rect(panel.x + panel.width - 166f, panel.y + 24f, 148f, 50f), "BEGIN BATTLE", buttonStyle))
                ConfirmTacticalDeployment();
        }
        ...
        private void OpenOperationalTacticalMap(HexCellView inspected, OperationalBattalionState parentBattalion, OperationalContactState contact)
        {
            if (parentBattalion != null) SelectOperationalBattalion(operationalFriendlyViews[parentBattalion.Id]);
            tacticalOperationalEngagement = true;
            tacticalForcePanelOpen = false;
            tacticalDeploymentActive = false;
            tacticalOperationalEnemyBattalion = contact == null
                ? null
                : operationalEnemyBattalions.Find(candidate => candidate.Id == contact.TargetId);
            RequestTacticalMap(inspected, tacticalOperationalEnemyBattalion != null);
        }
        ...
        private string BattalionOrderOfBattleText(OperationalBattalionState battalion, bool includeLocation)
        {
            ...
            return $"{battalion.DisplayName}\n{(includeLocation ? battalion.Position + "  •  " : string.Empty)}Strength {battalion.Strength}%  •  {battalion.Platoons.Count} platoons\n{subordinates}";
        }
        ...
        private void DrawSettingsButtonAndPanel(float uiWidth, float uiHeight)
        {
            ...
            if (awaitingRemapFor != null && Event.current.type == EventType.KeyDown && Event.current.keyCode != KeyCode.None)
            {
                ...
                Event.current.Use();
                bool reserved = pressed == KeyCode.H;
                ...
                if (reserved)
                {
                    remapRejectionText = "H is reserved for the HUD visibility toggle";
                }
                else if (collision)
                {
        ...
        private void DrawTacticalInterface(float uiWidth, float uiHeight, bool contextOnly = false)
        {
            if (!contextOnly)
            {
            ...
            string conditions = tacticalBattlefield.QuickBattle != null && activeBattleRequest == null
                ? $" • {tacticalBattlefield.QuickBattle.Weather.ToString().ToUpperInvariant()} • {tacticalBattlefield.QuickBattle.Visibility.ToString().ToUpperInvariant()}"
                : string.Empty;
            GUI.Label(new Rect(38f, 94f, 360f, 25f), tacticalBattlefield.BattlefieldId + conditions, unitNameStyle);
            ...
            stateStyle.normal.textColor = tacticalUnitState.Readiness == UnitReadiness.Moving
                ...
                : tacticalUnitState.Readiness == UnitReadiness.Spent
                    ? new Color(.48f, .52f, .48f)
                    : new Color(.96f, .73f, .20f);
            string readinessLabel = tacticalUnitState.IsConcealed ? "CONCEALED" :
                tacticalUnitState.MoraleState != TacticalMoraleState.Steady ? tacticalUnitState.MoraleState.ToString().ToUpperInvariant() :
                tacticalUnitState.Readiness.ToString().ToUpperInvariant();
            GUI.Label(new Rect(270f, 166f, 117f, 22f), readinessLabel, stateStyle);
            ...
            string engineerStores = tacticalUnitState.MaximumEngineerSupply > 0
                ? $" • ENG {tacticalUnitState.EngineerSupply}/{tacticalUnitState.MaximumEngineerSupply}" : string.Empty;
            string mortarStores = tacticalUnitState.MaximumMortarRounds > 0
                ? $" • MTR {tacticalUnitState.MortarRounds}/{tacticalUnitState.MaximumMortarRounds}{(tacticalUnitState.MortarIsDeployed ? " SET" : string.Empty)}" : string.Empty;
            GUI.Label(new Rect(38f, 183f, 350f, 14f),
                $"STR {tacticalUnitState.Strength}/{tacticalUnitState.MaximumStrength} • COH {tacticalUnitState.CombatStatus.ToString().ToUpperInvariant()} {tacticalUnitState.SuppressionPoints} • SUP {tacticalUnitState.Supply} • POS {tacticalUnitState.EntrenchmentLevel}{engineerStores}{mortarStores}", badgeStyle);
            ...
            bool tacticalControlsEnabled = !mapTransitionActive && !tacticalUnitMoving && !tacticalFireResolving && !tacticalEnemyTurnActive && !campaignBriefingActive && !settingsPanelOpen && !supportCardModalActive && !tacticalForcePanelOpen && !tacticalDeploymentActive;
            ...
            int visibleContactLines = Math.Min(6, tacticalContacts.Count);
            int overflowContactLines = Math.Max(0, tacticalContacts.Count - visibleContactLines);
            Rect intelligenceRect = new Rect(uiWidth - 355f, 18f, 335f,
                151f + visibleContactLines * 25f + (overflowContactLines > 0 ? 22f : 0f) + (showSpottedBadge ? 30f : 0f) + 68f);
            ...
            GUI.Label(new Rect(intelligenceRect.x + 16f, intelligenceRect.y + 36f, 300f, 38f),
                $"OBSERVER • {tacticalUnitState.DisplayName.ToUpperInvariant()}\nCOMPANY LOS + TERRAIN SENSOR PICTURE", bodyStyle);
            int contactLine = 0;
            foreach (TacticalUnitState enemy in tacticalEnemyStates)
            {
                if (contactLine >= visibleContactLines) break;
            ...
            float contactFooterY = intelligenceRect.y + 82f + visibleContactLines * 25f;
            if (overflowContactLines > 0)
            {
                GUI.Label(new Rect(intelligenceRect.x + 16f, contactFooterY, 300f, 20f),
                    $"+ {overflowContactLines} ADDITIONAL CONTACTS | USE MAP MARKERS", badgeStyle);
                contactFooterY += 22f;
            }
            Rect speedRect = new Rect(intelligenceRect.x + 16f, contactFooterY, 300f, 28f);
            ...
            GUI.Label(new Rect(intelligenceRect.x + 16f, nextRowY, 300f, 22f),
                reactionReady
                    ? $"REACTION • {tacticalUnitState.ReactionPolicy.ToString().ToUpperInvariant()} • {tacticalUnitState.ReactionPoints}/{TacticalFireAndManeuver.ReactionPointMaximum} RP • FACE {tacticalUnitState.FacingSector + 1}"
                    : $"REACTION • {tacticalUnitState.ReactionPolicy.ToString().ToUpperInvariant()} • UNAVAILABLE", reactionStyle);
            nextRowY += 30f;
            ...
            DrawTacticalCompanyList(uiWidth, uiHeight, intelligenceRect.yMax + 10f, tacticalControlsEnabled);
            ...
            if (hoveredLocalCell != null)
            {
                ...
                TacticalObstacleState inspectedObstacle = FindTacticalObstacle(hoveredLocalCell.Coord);
                bool showObstacle = inspectedObstacle != null && inspectedObstacle.UsmcIntelligence != TacticalObstacleIntelligence.Hidden;
            ...
            if (tacticalMenuOpen)
            {
                ...
                GUI.Label(new Rect(tacticalMenuRect.x + 12f, tacticalMenuRect.y + 7f, 154f, 22f), TacticalCompany.ShortRole(InferFriendlyRole(tacticalUnitState.Id)), badgeStyle);
                GUI.enabled = tacticalUnitState.CanMove;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 34f, 158f, 28f),
                        tacticalUnitState.CanMove ? $"MOVE • {tacticalUnitState.RemainingActionPoints} AP" : "MOVE • SPENT",
                        "MOVE | Choose a destination and preview its route and AP cost. Movement speed and exposure depend on the selected posture."))
                    BeginTacticalMovePlanning();
                GUI.enabled = true;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 67f, 158f, 28f), "INSPECT LOS",
                        "INSPECT LOS | Check whether one hex is visible from this platoon. The preview explains terrain, cover, smoke, and elevation effects."))
                    BeginTacticalLosPlanning();
                ...
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 100f, 158f, 28f),
                        tacticalWeapon.RemainingAmmunition > 0 ? $"DIRECT FIRE • {tacticalWeapon.RemainingAmmunition}" : "DIRECT FIRE • EMPTY",
                        "DIRECT FIRE | Attack an observed enemy platoon. Accuracy, suppression, casualties, AP, and ammunition depend on range, terrain, and attack type."))
                    BeginTacticalFirePlanning();
                GUI.enabled = tacticalUnitState.CanRally;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 133f, 158f, 28f),
                        tacticalUnitState.CombatStatus == TacticalCombatStatus.Ready ? "RALLY • READY" : $"RALLY • {tacticalUnitState.CombatStatus.ToString().ToUpperInvariant()}",
                        "RALLY | Spend AP to remove suppression and improve cohesion. It is useful before moving, firing, or assaulting with a shaken platoon."))
                    IssueTacticalRally();
                int reconCost = ActiveReconActionPointCost();
                GUI.enabled = tacticalUnitState.RemainingActionPoints >= reconCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 166f, 158f, 28f),
                        $"RECON • {reconCost} AP",
                        "RECON | Task sensors against a selected area without requiring LOS. Contacts within the search radius receive improved identification for two turns."))
                    BeginTacticalReconPlanning();
                GUI.enabled = true;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 199f, 158f, 28f),
                        tacticalLosOverlayActive ? "LOS OVERLAY • ON" : "LOS OVERLAY • OFF",
                        "LOS OVERLAY | Tint all nearby hexes as clear, obscured, or blocked from this platoon's current position. This costs no AP."))
                    ToggleTacticalLosOverlay();
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 232f, 158f, 28f),
                        $"POSTURE • {tacticalUnitState.MovementPosture.ToString().ToUpperInvariant()}",
                        "POSTURE | Cycle Quick, Tactical, or Bounding movement. Quick travels farther but is exposed; Bounding is slower and preserves reaction capability."))
                    CycleMovementPosture();
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 265f, 158f, 28f),
                        $"REACTION • {tacticalUnitState.ReactionPolicy.ToString().ToUpperInvariant()}",
                        "REACTION | Cycle Weapons Hold, Return Fire, or Weapons Free. The policy controls when reserved reaction points may interrupt enemy movement."))
                    CycleReactionPolicy();
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 298f, 158f, 28f), "FACE SECTOR",
                        "FACE SECTOR | Turn the platoon's covered arc toward a selected hex. Reaction fire is strongest to the front and weaker from flank or rear aspects."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.Face);
                GUI.enabled = tacticalUnitState.CanFire && tacticalUnitState.RemainingActionPoints >= TacticalFireAndManeuver.SuppressionActionPointCost && tacticalWeapon.RemainingAmmunition >= TacticalFireAndManeuver.SuppressionAmmunitionCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 331f, 158f, 28f), "AREA SUPPRESS",
                        "AREA SUPPRESS | Fire into a terrain hex rather than a specific observed target. It spends extra ammunition to pin suspected or concealed defenders."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.AreaSuppress);
                GUI.enabled = tacticalUnitState.CanMove && tacticalUnitState.RemainingActionPoints >= TacticalFireAndManeuver.SmokeActionPointCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 364f, 158f, 28f), "DEPLOY SMOKE",
                        "DEPLOY SMOKE | Place a temporary smoke screen within range. Smoke blocks or degrades observation and helps cover movement or disengagement."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.Smoke);
                ...
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 397f, 158f, 28f), "CLOSE ASSAULT",
                        "CLOSE ASSAULT | Attack an adjacent enemy position. Suppress the defender first; cover, cohesion, strength, and the Aggressive trait strongly affect the result."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.Assault);
                GUI.enabled = true;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 430f, 158f, 28f),
                        $"ATTACK • {tacticalAttackType.ToString().ToUpperInvariant()}",
                        "ATTACK TYPE | Cycle Rapid, Hasty, Standard, or Deliberate fire. Faster attacks cost less time; deliberate fire costs more AP for better effect."))
                    CycleAttackType();
                ...
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 463f, 158f, 28f),
                        $"ENTRENCH • L{tacticalUnitState.EntrenchmentLevel}",
                        "ENTRENCH | Spend AP and supply to improve this platoon's fighting position. Higher levels reduce incoming fire and improve staying power."))
                    IssueEntrench();
                GUI.enabled = TacticalPlannedFires.CanOrder(tacticalUnitState, tacticalBattlefield.FireSupport,
                    tacticalUnitState.Position, turnState.TurnNumber, false, out _);
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 496f, 158f, 28f),
                        $"CALL {FireMissionShortLabel(tacticalFireMissionType)} • {tacticalBattlefield.FireSupport.Ammunition}",
                        "CALL FOR FIRE | Request the selected HE, smoke, or illumination mission. It arrives next turn, may scatter, and consumes limited support ammunition."))
                ...
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 529f, 158f, 28f), "RESUPPLY",
                        "RESUPPLY | Replenish ammunition and supply while beside a friendly Weapons Platoon. The receiving platoon must have enough AP to complete the transfer."))
                    IssueResupply();
                ...
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 10f, tacticalMenuRect.y + 562f, 158f, 28f), "REORGANIZE",
                        "REORGANIZE | Spend AP and supply to restore some lasting combat strength after casualties. This is slower than Rally and repairs strength, not suppression."))
                    IssueReorganize();
                localMovementBoard.TryGetValue(tacticalUnitState.Position, out TacticalMovementCell concealmentCell);
                GUI.enabled = TacticalConcealmentMorale.CanHide(tacticalUnitState, concealmentCell);
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 34f, 158f, 28f),
                        tacticalUnitState.IsConcealed ? "HIDE - CONCEALED" : "HIDE - 2 AP",
                        "HIDE | Use suitable terrain and AP to conceal the platoon. Moving or firing may reveal it; concealed units can gain an ambush advantage."))
                    IssueHide();
                GUI.enabled = tacticalUnitState.CanMove && tacticalUnitState.RemainingActionPoints >= TacticalConcealmentMorale.SearchActionPointCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 67f, 158f, 28f), "SEARCH / CLEAR",
                        "SEARCH / CLEAR | Search an adjacent hex for concealed enemies or hidden obstacles. Use this before committing the main body through suspicious terrain."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.Search);
                GUI.enabled = tacticalUnitState.CanMove && tacticalUnitState.RemainingActionPoints >= TacticalConcealmentMorale.WithdrawActionPointCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 100f, 158f, 28f), "WITHDRAW",
                        "WITHDRAW | Fall back from enemy pressure toward a selected safer hex. This costs AP but can prevent a suppressed platoon from breaking or surrendering."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.Withdraw);
                bool isEngineer = InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Engineers;
                GUI.enabled = isEngineer && tacticalUnitState.CanMove && tacticalUnitState.RemainingActionPoints >= TacticalObstacles.DetectActionPointCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 133f, 158f, 28f), "DETECT / MARK",
                        "DETECT / MARK | Engineers inspect and mark a nearby minefield, wire belt, or roadblock, making the obstacle visible and safer to address."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.DetectObstacle);
                GUI.enabled = isEngineer && tacticalUnitState.CanMove && tacticalUnitState.RemainingActionPoints >= TacticalObstacles.HastyBreachActionPointCost &&
                    tacticalUnitState.EngineerSupply >= TacticalObstacles.HastyBreachSupplyCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 166f, 158f, 28f), $"HASTY BREACH • {tacticalUnitState.EngineerSupply}",
                        "HASTY BREACH | Engineers rapidly attempt to open an adjacent obstacle. It uses fewer AP but has more risk and creates a less dependable lane."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.HastyBreach);
                GUI.enabled = isEngineer && tacticalUnitState.CanMove && tacticalUnitState.RemainingActionPoints >= TacticalObstacles.DeliberateBreachActionPointCost &&
                    tacticalUnitState.EngineerSupply >= TacticalObstacles.DeliberateBreachSupplyCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 199f, 158f, 28f), $"DELIB BREACH • {tacticalUnitState.EngineerSupply}",
                        "DELIBERATE BREACH | Engineers spend more AP and stores to methodically open an adjacent obstacle, with a better chance of creating a safe lane."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.DeliberateBreach);
                GUI.enabled = true;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 232f, 158f, 28f),
                        $"FIRE TYPE • {FireMissionShortLabel(tacticalFireMissionType)}",
                        "FIRE TYPE | Select the payload for the next support or mortar request: HE attacks troops, smoke blocks observation, and illumination reveals night terrain."))
                    CycleFireMissionType();
                GUI.enabled = tacticalUnitState.RemainingActionPoints >= TacticalPlannedFires.AdjustActionPointCost &&
                    tacticalBattlefield.PlannedFireMissions.Count > 0;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 265f, 158f, 28f), "ADJUST FIRE • 1 AP",
                        "ADJUST FIRE | Use the previous aim point to improve the accuracy of a later mission. Adjustment costs 1 AP and reduces likely scatter."))
                    IssueAdjustFire();
                bool isWeapons = InferFriendlyRole(tacticalUnitState.Id) == TacticalPlatoonRole.Weapons;
                GUI.enabled = isWeapons && !tacticalUnitState.MortarIsDeployed && tacticalUnitState.RemainingActionPoints >= TacticalPlannedFires.MortarSetupActionPointCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 298f, 158f, 28f), "SET UP MORTARS",
                        "SET UP MORTARS | Emplace a Weapons Platoon's organic mortars. The platoon must not have moved or fired; displacement is required before moving again."))
                    IssueMortarSetup();
                GUI.enabled = isWeapons && tacticalUnitState.MortarIsDeployed && tacticalUnitState.MortarRounds > 0 &&
                    tacticalUnitState.RemainingActionPoints >= TacticalPlannedFires.MortarFireActionPointCost;
                if (DrawOrderButton(new Rect(tacticalMenuRect.x + 178f, tacticalMenuRect.y + 331f, 158f, 28f),
                        $"MORTAR {FireMissionShortLabel(SelectedMortarMissionType())} • {tacticalUnitState.MortarRounds}",
                        "MORTAR FIRE | Order this emplaced Weapons Platoon's organic mortar mission. It uses its own limited rounds rather than the supporting-fire pool."))
                    BeginTacticalSpecialOrder(TacticalSpecialOrder.MortarFire);
                GUI.enabled = true;
            }
            if (!contextOnly)
            {
        ...
        private void DrawTacticalCenterNotices(float uiWidth, float uiHeight)
        {
            if (tacticalBattlefield?.Tutorial != null && tacticalBattlefield.Tutorial.Enabled && !tacticalBattlefield.Tutorial.Completed)
            {
                Rect tutorialRect = new Rect(uiWidth / 2f - 240f, uiHeight - 154f, 480f, 48f);
                GUI.Box(tutorialRect, GUIContent.none);
                GUI.Label(new Rect(tutorialRect.x + 14f, tutorialRect.y + 8f, tutorialRect.width - 28f, 32f),
                    $"TRAINING {tacticalBattlefield.Tutorial.Step + 1}/6 • {TacticalStandaloneRules.TutorialPrompt(tacticalBattlefield.Tutorial.Step)}", bodyStyle);
            }
        ...
        private void DrawTacticalCompanyList(float uiWidth, float uiHeight, float requestedY, bool controlsEnabled)
        {
            float contentHeight = tacticalFriendlyStates.Count * 32f;
            float y = Mathf.Min(requestedY, uiHeight - 212f);
            float height = Mathf.Min(300f, Mathf.Max(100f, uiHeight - y - 112f));
            Rect panel = new Rect(uiWidth - 355f, y, 335f, height);
            GUI.Box(panel, GUIContent.none);
            GUI.Label(new Rect(panel.x + 16f, panel.y + 10f, 300f, 22f),
                tacticalOperationalEngagement ? "BATTALION | N NEXT | TAB NEXT ACTIONABLE" : "COMPANY | N NEXT | TAB NEXT ACTIONABLE", badgeStyle);
            Rect viewport = new Rect(panel.x + 8f, panel.y + 36f, panel.width - 16f, panel.height - 44f);
            Rect content = new Rect(0f, 0f, viewport.width - 18f, Math.Max(viewport.height, contentHeight));
            tacticalCompanyScroll = GUI.BeginScrollView(viewport, tacticalCompanyScroll, content);
            for (int index = 0; index < tacticalFriendlyStates.Count; index++)
            {
                TacticalUnitState friendly = tacticalFriendlyStates[index];
                bool active = friendly == tacticalUnitState;
                string status = friendly.IsConcealed ? "CONCEALED" : friendly.MoraleState != TacticalMoraleState.Steady
                    ? friendly.MoraleState.ToString().ToUpperInvariant()
                    : friendly.CombatStatus == TacticalCombatStatus.Ready ? friendly.Readiness.ToString().ToUpperInvariant() : friendly.CombatStatus.ToString().ToUpperInvariant();
                string label = $"{(active ? "▶ " : string.Empty)}{friendly.DisplayName} • {friendly.RemainingActionPoints} AP • {status}";
                GUI.enabled = controlsEnabled;
                if (GUI.Button(new Rect(2f, index * 32f, content.width - 4f, 27f), label, buttonStyle))
                {
                    CancelTacticalInteraction(false);
                    SetActiveTacticalFriendly(friendly.Id, true);
                    tacticalOrderFeedback = friendly.DisplayName.ToUpperInvariant() + " SELECTED";
                    tacticalAudio.Play(TacticalSound.Select);
                }
                GUI.enabled = true;
            }
        ...
        private void DrawResultScreen(float uiWidth, float uiHeight)
        {
            ...
            if (tacticalBattlefield.AfterActionReport != null)
            {
                TacticalAfterActionReport report = tacticalBattlefield.AfterActionReport;
                summary += $"\nCombat power USMC {report.FriendlyStrengthRemaining} • PLA {report.EnemyStrengthRemaining} • Ammo expended {report.AmmunitionExpended} • Secondary {report.SecondaryObjectivesCompleted}/{tacticalBattlefield.SecondaryObjectives.Count}";
            }
        ...
        private string CellInspectionText(HexCellView cell, string heading)
        {
            ...
            TacticalObstacleState obstacle = tacticalMode ? FindTacticalObstacle(cell.Coord) : null;
            if (obstacle != null && obstacle.UsmcIntelligence != TacticalObstacleIntelligence.Hidden)
            {
                string identity = obstacle.UsmcIntelligence == TacticalObstacleIntelligence.Suspected ? "Suspected obstacle" :
                    obstacle.UsmcIntelligence == TacticalObstacleIntelligence.Detected ? "Detected obstacle" : obstacle.Type.ToString();
                int extraCost = TacticalObstacles.MovementCost(obstacle.Type, obstacle.UsmcIntelligence, obstacle.IsBreached);
                text += obstacle.IsBreached ? "\nBreached lane • Safe passage" : $"\n{identity} • +{extraCost} AP • Halts on entry";
            }
            if (tacticalMode)
                foreach (TacticalReconMarker marker in tacticalReconMarkers)
                {
            ...
            if (tacticalMode && TacticalPlannedFires.IsIlluminated(tacticalBattlefield?.ActiveIlluminationMarkers, cell.Coord, turnState.TurnNumber))
                text += "\nIlluminated • contacts here are easier to observe";
            if (tacticalMode && tacticalBattlefield?.PlannedFireMissions != null)
                foreach (TacticalPlannedFireMission mission in tacticalBattlefield.PlannedFireMissions)
                {
                    if (mission.Status != TacticalFireMissionStatus.Pending || !mission.RequestedHex.Equals(cell.Coord)) continue;
                    text += $"\nIncoming {FireMissionShortLabel(mission.Type)} • {Math.Max(0, mission.DeliveryTurn - turnState.TurnNumber)} turn countdown";
                    if (mission.DangerClose) text += " • DANGER CLOSE";
                    break;
                }
            return text;
        }
        ...
        private HexCoord FindOpenLocalDeploymentHex(HexCoord preferred, HashSet<HexCoord> occupied)
        {
            HexCoord best = preferred;
            int bestDistance = int.MaxValue;
            bool found = false;
            foreach (HexCellView cell in localCells.Values)
            {
                if (!cell.IsLand || occupied.Contains(cell.Coord)) continue;
                int distance = HexCoord.Distance(preferred, cell.Coord);
                if (found && (distance > bestDistance || distance == bestDistance &&
                    (cell.Coord.Q > best.Q || cell.Coord.Q == best.Q && cell.Coord.R >= best.R))) continue;
                best = cell.Coord;
                bestDistance = distance;
                found = true;
            }
            if (!found) throw new InvalidOperationException("The tactical map contains no open deployment hex.");
            return best;
        }
